  ...
  try {
    ...
    if (!overwrite) {
      packagingUtils.endWithMessage(
        `Destination or temp folder ${dir} exists. To overwrite for upgrade, run with flag --overwrite`);
    }
  } catch (e) {
    if (e.code != 'ENOENT') {
      packagingUtils.endWithMessage(`Could not stat destination or temp folder ${dir}. Error=${e.message}`);
    }
...
function extract(appPath, appDir) {
  yauzl.open(appPath, {lazyEntries: true}, function(err, zipfile) {
    if (err) {
      packagingUtils.endWithMessage(`Could not open App ${userInput.inputApp}. Error=${err.message}`);
    }
      ...
      try {
        rmrf(appDir, (err)=> {
          if (err) {
            cleanup();
            packagingUtils.endWithMessage(
              `Could not remove old App for upgrade, in folder ${appDir}. Error=${err.message}`);
          }
        ...
        packagingUtils.endWithMessage(
          `Could not rename temp folder ${appDir+'.temp'} to ${appDir}. Error=${e.message}`);
      }
    });  
    zipfile.readEntry();
    zipfile.on("entry", function(entry) {
      if (entry.fileName.endsWith('/')) {
        ...
        packagingUtils.mkdirp(path.join(appDir+'.temp',entry.fileName), {mode: DIR_WRITE_MODE}).then(()=> {
          ...
          cleanup();
          packagingUtils.endWithMessage(
            `App could not be installed, extraction failed during directory creation. Error=${err}`);
        });
      } else {
        //file
        packagingUtils.mkdirp(path.join(appDir+'.temp',path.dirname(entry.fileName)), {mode: DIR_WRITE_MODE}).then(()=>{
          zipfile.openReadStream(entry, function(err, readStream) {
            if (err) {
              cleanup();
              packagingUtils.endWithMessage(
                `App could not be installed, extraction failed during archived file read. Error=${err}`);
            }
...
if (!isFile(userInput.inputApp)) {
  ...
  extract(userInput.inputApp, appDir);
}
