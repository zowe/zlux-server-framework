...
DependencyGraph.prototype = {
  ...
  _buildGraph() {
    const g = {};
    let brokenDeps = [];
    let values = Object.keys(this.pluginsById).map((e)=> { return this.pluginsById[e];});
  ...
  _toposort(graph) {
    ...
    const pluginsSorted = [];
    let time = 0;
    let values = Object.keys(graph).map((e)=> { return graph[e];});
    for (let importedPlugin of values) {
  ...
  processImports() {
    ...
    let rejectValues = Object.keys(rejects).map((e)=> { return rejects[e];});
    return {
      plugins: pluginsSortedAndFiltered,
      rejects: rejectValues
    }
  }
}
