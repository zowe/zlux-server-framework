...
PluginLoader.prototype = {
  ...
  installPlugins(pluginDefs) {
    ...
    let newPlugins = [];
    let plugins = sortedAndRejectedPlugins.plugins;
    let loadedPluginCount = 0;
    for (let i = 0; i < pluginDefs.length; i++) {
      try {
        if (this.pluginMap[plugins[i].identifier]) {
        ...
        const pluginConfiguration = configService.getPluginConfiguration(
          plugins[i].identifier, plugins[i].location,
          this.options.serverConfig, this.options.productCode);
        bootstrapLogger.debug(`For plugin with id=${plugins[i].identifier}, internal config`
                                + ` found=\n${JSON.stringify(pluginConfiguration)}`);

        const plugin = makePlugin(plugins[i], pluginConfiguration, pluginContext,
                                  false, this.options.langManagers);
        if (plugin) {
          loadedPluginCount++;
          bootstrapLogger.log(bootstrapLogger.INFO,
            `Plugin ${plugin.identifier} at path=${plugin.location} loaded. (${loadedPluginCount}/${plugins.length})\n`);
          ...
          bootstrapLogger.log(bootstrapLogger.INFO,
            `Plugin ${plugins[i].identifier} not loaded`);
        }
      } catch (e) {
        console.log(e);
        //bootstrapLogger.warn(e)
        bootstrapLogger.log(bootstrapLogger.INFO,
          `Failed to load ${plugins[i].identifier}: ${e}`);
      }
    }
    this.registerStaticPluginsWithManagers(plugins);
