    ...
    for (const pluginDef of sortedAndRejectedPlugins.plugins) { 
      try {
        ...
        if (plugin) {
          ...
          plugin.recognizers = installApp.loadRecognizers(plugin.location, plugin.identifier);
          plugin.actions = installApp.loadActions(plugin.location, plugin.identifier);
