...
PluginLoader.prototype = {
  ...
  async installPlugins(pluginDefs) {
    ...
    envComps = await this.getComponentCapabilities(pluginContext.config); // Gets the environment and server information
          ...
          if (envComps[id]) { // ...if there exists a component the server could get environment information for,
            ...
            if (requiredComponents[id]) { //...and that component is one of the components a plugin has requirements for, check them.
              ...
              if (environmentComponent.os && requiredOS && requiredOS.length > 0 && environmentComponent.os) { // Plugin has manually required their OS support
                ...
                else if (acceptedOS.includes("!"+environmentComponent.os)) {
                  plugin.error = new Error(`(${environmentComponent.os}) is not a supported platform for ${name}. Skipping (${plugin.identifier})... Supported: ${acceptedOS}`);
                }
                else {
                  bootstrapLogger.warn("ZWED0173W", name, environmentComponent.os, plugin.identifier); //`(${name} could not verify (${environmentComponent.os}) as a supported platform to install (${plugin.identifier}). Proceeding anyway...`);
                }
              }
              ...
              if (environmentComponent.cpu && requiredCPU && requiredCPU.length > 0 && environmentComponent.cpu) { // Manually required arch
                if (requiredCPU.includes(environmentComponent.cpu)) {
                  ...
                  plugin.error = new Error(`(${environmentComponent.cpu}) is not a supported architecture for ${name}. Skipping (${plugin.identifier})... Supported: ${requiredCPU}`);
                }
                else {
                  bootstrapLogger.warn("ZWED0174W", name, environmentComponent.cpu, plugin.identifier);//(`${name} could not verify (${environmentComponent.cpu}) as a supported architecture to install (${plugin.identifier}). Proceeding anyway...`);
                }
              }
              ...
              if (requiredEndpoints && requiredEndpoints.length > 0 && environmentComponent.endpoints) { // Manually required endpoints
                for (let i = 0; i < requiredEndpoints.length; i++) {
                  if (!environmentComponent.endpoints.includes(requiredEndpoints[i])) {
                    plugin.error = new Error(`(${requiredEndpoints[i]}) is not a supported endpoint for ${name}. Skipping (${plugin.identifier})... Supported: ${environmentComponent.endpoints}`);
                  } else {
