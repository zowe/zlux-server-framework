...
//        }
        ...
        bootstrapLogger.info(`Seeking language manager for type=${dataservice.type}`);
        for (let i = 0; i < langManagers.length; i++) {
          const langManager = langManagers[i];
          if (langManager.getSupportedTypes().includes(dataservice.type)) {
            addService(dataservice, dataservice.name, this.dataServicesGrouped);
            bootstrapLogger.info(`${this.identifier}: `
...
PluginLoader.prototype = {
  ...
  installPlugins(pluginDefs) {
    ...
    this.registerStaticPluginsWithManagers(pluginDefs);
