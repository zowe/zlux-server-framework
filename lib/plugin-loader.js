...
function makeDataService(def, plugin, context) {
    const configuration = configService.getServiceConfiguration(plugin.identifier, def.name, context.config, context.productCode);
    let dataservice;
...
class LibraryPlugIn extends Plugin {
    ...
    init(context) {
        ...
        if (!fs.existsSync(this.location)) {
            bootstrapLogger.log(bootstrapLogger.WARNING, `${this.identifier}: library path ${this.location} does not exist`);
            return;
        }
        bootstrapLogger.log(bootstrapLogger.INFO, `Plugin ${this.identifier} will serve library data from directory ${this.location}`);
    }
}
;
...
class NodeAuthenticationPlugIn extends Plugin {
    ...
    isValid(context) {
        ...
        if (!context.authManager.authPluginRequested(this.identifier, this.authenticationCategory)) {
    ...
    init(context) {
        ...
        bootstrapLogger.log(bootstrapLogger.INFO, `Auth plugin ${this.identifier}: loading auth handler module ${filepath}`);
...
class PluginLoader extends EventEmitter {
    ...
    _readPluginDef(pluginDescriptorFilename) {
        ...
        if (!path.isAbsolute(pluginBasePath)) {
            pluginBasePath = this.options.relativePathResolver(pluginBasePath, "plugin-def");
        }
    ...
    readPluginDefs() {
        const defs = [];
        bootstrapLogger.log(bootstrapLogger.INFO, `Reading plugins dir ${this.options.pluginsDir}`);
        ...
        for (const pluginDescriptorFilename of pluginLocationJSONs) {
            try {
                ...
                bootstrapLogger.log(bootstrapLogger.INFO, `Failed to load ${pluginDescriptorFilename}\n`);
            }
        }
        return defs;
    }
    ...
    installPlugins(pluginDefs) {
        ...
        for (const rejectedPlugin of sortedAndRejectedPlugins.rejects) {
            bootstrapLogger.warn(`Could not initialize plugin`
                ...
                + zluxUtil.formatErrorStatus(rejectedPlugin.validationError, DependencyGraph.statuses));
        }
        for (const pluginDef of sortedAndRejectedPlugins.plugins) {
            try {
                const pluginConfiguration = configService.getPluginConfiguration(pluginDef.identifier, this.options.serverConfig, this.options.productCode);
                bootstrapLogger.debug(`For plugin with id=${pluginDef.identifier}, internal config`
                    + ` found=\n${JSON.stringify(pluginConfiguration)}`);
                const plugin = makePlugin(pluginDef, pluginConfiguration, pluginContext, false);
                bootstrapLogger.log(bootstrapLogger.INFO, `Plugin ${plugin.identifier} at path=${plugin.location} loaded.\n`);
                ...
                console.log(e);
                bootstrapLogger.log(bootstrapLogger.INFO, `Failed to load ${pluginDef.identifier}: ${e}`);
            }
        }
    ...
    addDynamicPlugin(pluginDef) {
        ...
        const pluginConfiguration = configService.getPluginConfiguration(pluginDef.identifier, this.options.serverConfig, this.options.productCode);
        const plugin = makePlugin(pluginDef, pluginConfiguration, pluginContext, true);
