...
function ExternalService(def, configuration, plugin, zoweConfig) {
  Service.call(this, def, configuration, plugin);
  if (!this.host) {
  ...
  if(this.host == 'agent') {
    const agentOptions = zluxUtil.getAgentRequestOptions(zoweConfig, {}, false);
    if (agentOptions) {
      this.host = agentOptions.host;
      this.port = agentOptions.port;
      this.isHttps = agentOptions.protocol == 'https:';
    }
  }

  if(this.host == 'zlux') {
    this.host = zluxUtil.getBestHostname(zoweConfig);
    this.port = zluxUtil.getBestPort(zoweConfig);
    this.isHttps = zluxUtil.isServerHttps(zoweConfig);
  }
}
...
function makeDataService(def, plugin, context) {
  ...
  if (def.type == "external") {
    dataservice = new ExternalService(def, configuration, plugin, context.zoweConfig);
  ...
  verifyStaticWebContent() {
    if (this.webContent) {
      ...
      if (!fs.existsSync(contentPath)) {
        ...
        bootstrapLogger.debug(`ZWED0036I`, this.identifier, contentPath); //bootstrapLogger.info(`plugin ${this.identifier} ` + `will serve static files from ${contentPath}`);
      }
    }
  },
...
//          continue;
//        }
        dataservice.loadImplementation(this.location);
  ...
  getApiCatalog(zoweConfig) {
    return makeSwaggerCatalog(this, zluxUtil.getProductCode(zoweConfig), zoweConfig);
  }
...
function NodeAuthenticationPlugIn(def, configuration) {
  Plugin.call(this, def, configuration);
  if (!this.authenticationCategory && Array.isArray(this.authenticationCategories)
      && this.authenticationCategories.length > 0) {
    this.authenticationCategory = this.authenticationCategories[0];
  }
}
...
PluginLoader.prototype = {
  ...
  _readPluginDef(pluginDescriptorFilename) {
    const pluginPtrPath = zluxUtil.normalizePath(pluginDescriptorFilename,
    ...
    if (!path.isAbsolute(pluginBasePath)) {
      ...
      pluginBasePath = zluxUtil.normalizePath(pluginBasePath, relativeTo);
    }
  ...
  async loadPlugins() {
    ...
    return this.installPlugins(defs);
  },
  ...
  async installPlugins(pluginDefs) {
    const pluginContext = {
      ...
      config: this.options.serverConfig.components['app-server'],
      zoweConfig: this.options.serverConfig,
    ...
    if (Object.keys(envComps).length === 0) {
      await this.getComponentCapabilities(this.options.serverConfig); // Gets the environment and server information
    }
