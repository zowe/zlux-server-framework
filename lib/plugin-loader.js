...
PluginLoader.prototype = {
  ...
  getComponentCapabilities(config) {
    return new Promise((complete, fail)=> {
      ...
      if (!host || !port) {
        ...
        let url = protocol + host + ':' + port + '/server/agent/environment';
        console.log(`request url agent api ${url}`);
        const httpApi = isHttps ? https : http;
        const requestOptions = isHttps ? this.tlsOptions : {};
        return new Promise((resolve, reject) => { /* Obtains and stores environment information from agent */
          httpApi.get(url, requestOptions, (res) => {
          ...
          url = protocol + host + ':' + port + '/server/agent/services';
  ...
  setTlsOptions: function (tlsOptions) {
