  ...
  getComponentCapabilities(config) {
    return new Promise((complete, fail)=> {
      ...
      if (!requestOptions) {
        ...
        const replacer = function (key, val) { return (Array.isArray(val) && val.length > 100) ? "<very long array>" : val; };
        console.log(`plugin loader request options:\n ${JSON.stringify(requestOptions, replacer, 2)}`);
        const dns = require('dns');
        dns.resolve(requestOptions.host, (err, records) => {
          if (err) {
            console.error(`unable to resolve ${requestOptions.host}`, err);
            return;
          }
          console.log(`'${requestOptions.host}' resolves to:\n${JSON.stringify(records)}`);
        });
