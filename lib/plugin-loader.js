...
PluginLoader.prototype = {
  ...
  installPlugins(pluginDefs) {
    ...
    for (const pluginDef of pluginDefs) {
      if (pluginDef.error) {
        ...
        this.checkPluginRequirements(pluginDef);
        if (pluginDef.error) {
          newPlugins.push(pluginDef);
        } else {
  ...
  checkPluginRequirements(plugin) {
    if (plugin.pluginReqs) {
      const components = plugin.pluginReqs.components;
      const capabilities = plugin.pluginReqs.capabilities;
      const appServerComp = 'org.zowe.core.zlux.zlux-core';
      const agentComp = 'org.zowe.core.zss';

      if (components) {
        if (components[appServerComp]) { /* --- App server component --- */
          const setOS = components[appServerComp].os && typeof (components[appServerComp].os) === 'set' ? 
            new Set(components[appServerComp].os) : null;
          // const setVersions = new Set(components[appServerComp].version);
          const setCPU = components[appServerComp].cpu && typeof (components[appServerComp].cpu) === 'set' ? 
            new Set(components[appServerComp].cpu) : null;

          if (setOS) { // Plugin has manually defined their OS support section
            if (setOS.has(process.platform)) {
              bootstrapLogger.debug('(' + process.platform + ') is a supported platform by the App server to install (' + plugin.identifier + ').');
            } 
            else if (setOS.has("!"+process.platform)) {
              plugin.error = new Error('(' + process.platform + ') is not a supported platform by the App server to install (' + plugin.identifier + '). Skipping Plugin...');
            }
            else {
              bootstrapLogger.warn('App server could not verify (' + process.platform + ') as a supported platform to install (' + plugin.identifier + '). Proceeding...');
            }
          }
          // if (setVersions) {
          //   //if (setVersions.has()) TODO - Check version of App server?
          // }
          if (setCPU) {
            if (setCPU.has(process.arch)) {
              bootstrapLogger.debug('(' + process.arch + ') is a supported architecture by the App server to install (' + plugin.identifier + ').');
            } else if (setCPU.has("!"+process.arch)) {
              plugin.error = new Error('(' + process.arch + ') is not a supported architecture by the App server to install (' + plugin.identifier + '). Skipping Plugin...');
            }
            else {
              bootstrapLogger.warn('App server could not verify (' + process.arch + ') as a supported architecture to install (' + plugin.identifier + '). Proceeding...');
            }
          }
        }

        if (components[agentComp]) { /* --- Agent component --- */
          const setOS = components[agentComp].os && typeof (components[agentComp].os) === 'set' ? 
            new Set(components[agentComp].os) : null;

          const setCPU = components[agentComp].cpu && typeof (components[agentComp].cpu) === 'set' ? 
            new Set(components[agentComp].cpu) : null;

          const setVersions = components[agentComp].version && typeof (components[agentComp].version) === 'set' ? 
            new Set(components[agentComp].version) : null;

          const endpoint = ''; // TODO: ZSS endpoint goes here for environment
