...
PluginLoader.prototype = {
  ...
  _readPluginDef(pluginDescriptorFilename) {
    ...
    if (!fs.existsSync(pluginBasePath)) {
      throw new Error(`ZWED0015E - ${pluginDescriptorFilename}: No plugin directory found at`
    ...
    if (!fs.existsSync(pluginDefPath)) {
      throw new Error(`ZWED0016E - ${pluginDescriptorFilename}: No pluginDefinition.json `
    ...
    if (pluginDef.identifier !== pluginPtrDef.identifier) {
      throw new Error(`ZWED0017E - ${pluginDef.identifier} and ${pluginPtrDef.identifier} `
          + `don't match - plugin ignored`);
    }
    if (!pluginDef.pluginType) {
      throw new Error(`ZWED0018E - No plugin type found for ${pluginDef.identifier} `
  ...
  installPlugins(pluginDefs) {
    ...
    for (const pluginDef of sortedAndRejectedPlugins.plugins) { 
      try {
        ...
        bootstrapLogger.warn("ZWED0035W", `${pluginDef.identifier}`, `${e}`); //bootstrapLogger.log(bootstrapLogger.INFO,
