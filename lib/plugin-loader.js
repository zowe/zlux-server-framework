...
const APP_SERVER_COMP_ID = 'org.zowe.core.zlux.zlux-core';
    
const AGENT_COMP_ID = 'org.zowe.core.zss';
...
PluginLoader.prototype = {
  ...
  async loadPlugins() {
    ...
    await this.installPlugins(defs);
  },
  ...
  async installPlugins(pluginDefs) {
    const pluginContext = {
    ...
    const componentCapabilities = await this.getComponentCapabilities(pluginContext.config); // Gets the environment and server information
      ...
      for (const pluginDef of pluginDefs) {
        if (pluginDef.error) {
          ...
          this.checkPluginRequirements(pluginDef, componentCapabilities); // Checks that the environment and server satisfies the plugin req's
          if (pluginDef.error) {
            newPlugins.push(pluginDef);
          } else {
  ...
  async getComponentCapabilities(config) {
    let appServerComp = {};
    let agentComp = {};
    let host;
    let port;
    ...
    if (host && port) {
      ...
      'http://' + host + ':' + port + '/server/agent/services';
      ...
      return new Promise((resolve, reject) => { 
        ...
        let obj = {};
        obj[AGENT_COMP_ID] = agentComp;
        obj[APP_SERVER_COMP_ID] = appServerComp;
        return obj;
      });
    });
  }
  ...
  checkPluginRequirements(plugin, components) {
    if (plugin.pluginReqs) {
      const acceptedComponents = plugin.pluginReqs.components;
      
      if (acceptedComponents) {
        if (acceptedComponents[APP_SERVER_COMP_ID]) { /* --- App server component --- */
          const acceptedOS = acceptedComponents[APP_SERVER_COMP_ID].os;
          const acceptedCPU = acceptedComponents[APP_SERVER_COMP_ID].cpu;

          if (acceptedOS && acceptedOS.length > 0 && components[APP_SERVER_COMP_ID].os) { // Plugin has manually defined their OS support section
            if (acceptedOS.includes(components[APP_SERVER_COMP_ID].os)) {
              console.log(`(${components[APP_SERVER_COMP_ID].os}) is a supported platform for the App server to install (${plugin.identifier}).`);
            } 
            else if (acceptedOS.includes("!"+components[APP_SERVER_COMP_ID].os)) {
              plugin.error = new Error(`(${components[APP_SERVER_COMP_ID].os}) is not a supported platform for the App server. Skipping (${plugin.identifier})... Supported: ${acceptedOS}`);
            }
            else {
              console.log(`App server could not verify (${components[APP_SERVER_COMP_ID].os}) as a supported platform to install (${plugin.identifier}). Proceeding anyway...`);
            }
          }

          if (acceptedCPU && acceptedCPU.length > 0 && components[APP_SERVER_COMP_ID].cpu) {
            if (acceptedCPU.includes(components[APP_SERVER_COMP_ID].cpu)) {
              console.log(`(${components[APP_SERVER_COMP_ID].cpu}) is a supported architecture for the App server to install (${plugin.identifier}).`);
            } else if (acceptedCPU.includes("!"+components[APP_SERVER_COMP_ID].cpu)) {
              plugin.error = new Error(`(${components[APP_SERVER_COMP_ID].cpu}) is not a supported architecture for the App server. Skipping (${plugin.identifier})... Supported: ${acceptedCPU}`);
            }
            else {
              console.log(`App server could not verify (${components[APP_SERVER_COMP_ID].cpu}) as a supported architecture to install (${plugin.identifier}). Proceeding anyway...`);
            }
          }
        }

        if (acceptedComponents[AGENT_COMP_ID]) { /* --- Agent component --- */
          const acceptedOS = acceptedComponents[AGENT_COMP_ID].os;
          const acceptedCPU = acceptedComponents[AGENT_COMP_ID].cpu;
          const setVersions = acceptedComponents[AGENT_COMP_ID].version;
          const neededEndpoints = acceptedComponents[AGENT_COMP_ID].endpoints;

          if (acceptedOS && acceptedOS.length > 0 && components[AGENT_COMP_ID].os) { // Plugin has manually defined their OS support section
            if (acceptedOS.includes(components[AGENT_COMP_ID].os)) {
              console.log(`(${components[AGENT_COMP_ID].os}) is a supported platform for the agent to install (${plugin.identifier}).`);
            } 
            else if (acceptedOS.includes("!"+components[AGENT_COMP_ID].os)) {
              plugin.error = new Error(`(${components[AGENT_COMP_ID].os}) is not a supported platform for the agent. Skipping (${plugin.identifier})... Supported: ${acceptedOS}`);
            }
            else {
              console.log(`Agent could not verify (${components[AGENT_COMP_ID].os}) as a supported platform to install (${plugin.identifier}). Proceeding anyway...`);
            }
          }
          if (setVersions && setVersions.length > 0 && components[AGENT_COMP_ID].version) {
            const actualVer = components[AGENT_COMP_ID].version.split('.');
            let supported = false;
            // TODO: Implement complex version verification
            // for (let i = 0; i < setVersions.length; i++) {
            //   let compareVer = setVersions[i].split('.');

            //   if (("!" + actualVer[0]).localeCompare(compareVer[0])) {
            //     if (actualVer[1].localeCompare(compareVer[1])) {
            //       supported = false;
            //       i = setVersions.length;
            //     }
            //   } else if ((">" + actualVer[0]).localeCompare(compareVer[0])) {
            //     if (actualVer[1].localeCompare(compareVer[1])) {
          ...
          if (acceptedCPU) {
            if (acceptedCPU.includes(components[AGENT_COMP_ID].cpu)) {
              console.log(`(${components[AGENT_COMP_ID].cpu}) is a supported architecture for the agent to install (${plugin.identifier}).`);
            } else if (acceptedCPU.includes("!"+components[AGENT_COMP_ID].cpu)) {
              plugin.error = new Error(`(${components[AGENT_COMP_ID].cpu}) is not a supported architecture for the agent. Skipping (${plugin.identifier})... Supported: ${acceptedCPU}`);
            }
            else {
              console.log(`Agent could not verify (${components[AGENT_COMP_ID].cpu}) as a supported architecture to install (${plugin.identifier}). Proceeding anyway...`);
            }
          }
          if (neededEndpoints && neededEndpoints.length > 0 && components[AGENT_COMP_ID].endpoints) {
            for (let i = 0; i < neededEndpoints.length; i++) {
              if (!components[AGENT_COMP_ID].endpoints.includes(neededEndpoints[i])) {
                plugin.error = new Error(`(${neededEndpoints[i]}) is not a supported endpoint for the agent. Skipping (${plugin.identifier})... Supported: ${components[AGENT_COMP_ID].endpoints}`);
              } else {
                console.log(`(${neededEndpoints[i]}) is a supported endpoint for the agent to install (${plugin.identifier}).`);
              }
            }
          }
        }
