...
const nodeSupportTimeTable = { // Signifies end of support
  6: new Date('2020-09-1'), // Order by highest vers first
}
...
checkNodeSupportVersion(installLog);
...
function checkNodeSupportVersion(logger) {
  var currentDate = new Date();

  for (var majorVer in nodeSupportTimeTable) {
    var months = nodeSupportTimeTable[majorVer].getMonth() - currentDate.getMonth() + 
      (12 * (nodeSupportTimeTable[majorVer].getFullYear() - currentDate.getFullYear()));

    if (parseInt(nodeMajorVer) <= majorVer) {
      if (currentDate > nodeSupportTimeTable[majorVer]) // We use break so if version is multiple iterations old, we don't get multiple messages
      {
        logger.warn("YOUR NODE VERSION IS OUT OF DATE. ZOWE IS NO LONGER SUPPORTING NODE VERSION " + majorVer + ".");
        break;
      } else if (months >= 6) {
        logger.info("Zowe will be ending support for Node version " + majorVer + " in the future.");
        break;
      } else if (months >= 2) {
        logger.warn("Zowe will be ending support for Node version " + majorVer + " in " + months + " months.");
        break;
      } else {
        logger.warn("Zowe will be ending support for Node version " + majorVer + " in a month or less.");
        break;
      }
    }
  }
