...
function DataserviceContext(serviceDefinition, serviceConfiguration, 
  ...
  if (process.clusterManager) { // Cluster mode
    createDataserviceStorage(this.plugin.pluginDef.identifier).then((data) => {
      ...
      let localStorage = this.storage.getStorage(this.plugin.pluginDef.identifier);
  ...
  return process.clusterManager.getStorage().then( function(clusterStorage) {
    ...
    clusterStorage.getStorage = function () {
      return process.clusterManager.getStorage().then((storage) => {
        return storage.storageDict[pluginId];
      });
    }

    clusterStorage.setStorageValue = function (id, value) {
      return process.clusterManager.setStorageValue(pluginId, id, value);
    }

    clusterStorage.setStorage = function (dict) {
      return process.clusterManager.setStorage(pluginId, dict);
    }

    clusterStorage.addStorage = function (dict) {
      return process.clusterManager.addStorage(pluginId, dict);
    }

    clusterStorage.getStorageValue = function (id) {
      return process.clusterManager.getStorageValue(pluginId, id);
    }

    clusterStorage.deleteStorageValue = function (id) {
      return process.clusterManager.deleteStorageValue(pluginId, id);
    }

    clusterStorage.setStorage(pluginId, []);

    contentLogger.info("'" + pluginId + "' context is loaded with storage object: ", clusterStorage);
