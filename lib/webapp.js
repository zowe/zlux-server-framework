    ...
    return function(req, res) {
      ...
      */
      ...
      if (options.serverConfig.dataserviceAuthentication.rbac) {
        if (req.username) {
          allowedPlugins = configService.getAllowedPlugins(configService.CONFIG_SCOPE_USER, options, req.username);
        } else {
          allowedPlugins = configService.getAllowedPlugins(configService.CONFIG_SCOPE_PRODUCT, options);
        }
  ...
  _installPluginStaticHandlers(plugin, urlBase) {
    ...
    if (plugin.pluginType === "library") {
      if (plugin.libraryVersion) {
        ...
        this.pluginRouter.use(url, expressStaticGzip(plugin.location,
                                                     {enableBrotli: true, orderPreference: ['br', 'gzip']}));
      } else {
