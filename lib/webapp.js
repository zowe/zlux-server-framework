...
class DataserviceContext {
    constructor(serviceDefinition, serviceConfiguration, pluginContext) {
        this.serviceDefinition = serviceDefinition;
        this.serviceConfiguration = serviceConfiguration;
        this.plugin = pluginContext;
        this.logger = global.COM_RS_COMMON_LOGGER.makeComponentLogger(pluginContext.pluginDef.identifier + "." + serviceDefinition.name);
    }
    makeSublogger(name) {
        return makeSubloggerFromDefinitions(this.plugin.pluginDef, this.serviceDefinition, name);
    }
...
exports.DataserviceContext = DataserviceContext;
;
    ...
    plugins: function (plugins) {
        return function (req, res) {
            ...
            */
            ...
            const acceptLanguage = translationUtils.getAcceptLanguageFromCookies(req.cookies) || req.headers['accept-language'] || '';
...
class WebServiceHandle {
    constructor(urlPrefix, environment) {
        this.urlPrefix = urlPrefix;
        if (!environment.loopbackConfig.port) {
            installLog.severe(`loopback configuration not valid,`, environment.loopbackConfig, `loopback calls will fail!`);
        }
...
exports.WebServiceHandle = WebServiceHandle;
;
const commonMiddleware = {
    ...
    addAppSpecificDataToRequest(globalAppData) {
        return function addAppSpecificData(req, res, next) {
            ...
            appData.webApp.callRootService = function callRootService(name, url, options) {
...
class WebApp {
    constructor(options) {
        this.installPlugin = BBPromise.coroutine(function* (pluginContext) {
            const plugin = pluginContext.pluginDef;
            const urlBase = zLuxUrl.makePluginURL(this.options.productCode, plugin.identifier);
            try {
    ...
    makeProxy(urlPrefix, noAuth, overrideOptions) {
        ...
        r.use(proxy.makeSimpleProxy(this.options.proxiedHost, this.options.proxiedPort, options));
        r.ws('/', proxy.makeWsProxy(this.options.proxiedHost, this.options.proxiedPort, urlPrefix, options.isHttps));
        return r;
    }
    ...
    installStaticHanders() {
        const webdir = path.join(path.join(this.options.productDir, this.options.productCode), 'web');
    ...
    installCommonMiddleware() {
        this.expressApp.use(commonMiddleware.addAppSpecificDataToRequest(this.appData));
    }
    ...
    installRootServices() {
        ...
        for (const proxiedRootService of this.options.rootServices || []) {
            ...
            if (proxiedRootService.requiresAuth === false) {
                ...
                this.expressApp.use(proxiedRootService.url, [commonMiddleware.logRootServiceCall(true, name), _router]);
            }
            else {
                const _router = this.makeProxy(proxiedRootService.url, false, getAgentProxyOptions(this.options, this.options.serverConfig.agent));
                this.expressApp.use(proxiedRootService.url, this.auth.middleware, [commonMiddleware.logRootServiceCall(true, name), _router]);
            }
            serviceHandleMap[name] = new WebServiceHandle(proxiedRootService.url, this.wsEnvironment);
        }
        this.expressApp.use(commonMiddleware.injectServiceHandles(serviceHandleMap, true));
        this._installRootService('/auth', 'post', this.auth.doLogin, { needJson: true, needAuth: false, isPseudoSso: true });
        this._installRootService('/auth', 'get', this.auth.getStatus, { needJson: true, needAuth: false, isPseudoSso: true });
        this._installRootService('/auth-refresh', 'get', this.auth.refreshStatus, { needJson: true, needAuth: false, isPseudoSso: true });
        this._installRootService('/auth-logout', 'post', this.auth.doLogout, { needJson: true, needAuth: false, isPseudoSso: true });
        this._installRootService('/auth-logout', 'get', this.auth.doLogout, { needJson: true, needAuth: false, isPseudoSso: true });
        serviceHandleMap['auth'] = new WebServiceHandle('/auth', this.wsEnvironment);
        this._installRootService('/plugins', 'get', staticHandlers.plugins(this.plugins), { needJson: false, needAuth: false, isPseudoSso: false });
        serviceHandleMap['plugins'] = new WebServiceHandle('/plugins', this.wsEnvironment);
        this._installRootService('/server/proxies', 'get', staticHandlers.proxies(this.options), { needJson: false, needAuth: true, isPseudoSso: false });
        serviceHandleMap['server/proxies'] = new WebServiceHandle('/server/proxies', this.wsEnvironment);
        this._installRootService('/echo/*', 'get', staticHandlers.echo(), { needJson: false, needAuth: true, isPseudoSso: false });
        serviceHandleMap['echo'] = new WebServiceHandle('/echo', this.wsEnvironment);
        this._installRootService('/apiManagement', 'use', staticHandlers.apiManagement(this), { needJson: false, needAuth: true, isPseudoSso: false });
        serviceHandleMap['apiManagement'] = new WebServiceHandle('/apiManagement', this.wsEnvironment);
    ...
    _makeRouterForLegacyService(pluginContext, service) {
        ...
        const handler = new constructor(service, service.methods, manager, legacyDataserviceAttributes);
    ...
    *_makeRouter(service, plugin, pluginContext, pluginChain) {
        const serviceRouterWithMiddleware = pluginChain.slice();
        serviceRouterWithMiddleware.push(commonMiddleware.injectServiceDef(service));
        serviceRouterWithMiddleware.push(this.auth.middleware);
        serviceRouterWithMiddleware.push(commonMiddleware.logServiceCall(plugin.identifier, service.name));
        ...
        switch (service.type) {
            case "service":
                ...
                router = this.makeProxy(zLuxUrl.makePluginURL(this.options.productCode, plugin.identifier) + zLuxUrl.makeServiceSubURL(service, false, true), false, getAgentProxyOptions(this.options, this.options.serverConfig.agent));
                break;
            case "nodeService":
                //installLog.info(
            ...
            case "router":
                {
                    ...
                    const serviceConfiguration = configService.getServiceConfiguration(plugin.identifier, service.name, pluginContext.server.config.app, this.options.productCode);
                    const dataserviceContext = new DataserviceContext(service, serviceConfiguration, pluginContext);
                    if (!service.routerFactory) {
                        ...
                        router = yield service.nodeModule[service.routerFactory](dataserviceContext);
            ...
            case "external":
                ...
                router = this.makeExternalProxy(service.host, service.port, service.urlPrefix, service.isHttps, undefined, plugin.identifier, service.name);
                break;
        }
    ...
    _makeServiceHandleMap(plugin, urlBase) {
        const serviceHandleMap = {};
        for (const group of zluxUtil.concatIterables(Object.values(plugin.dataServicesGrouped), Object.values(plugin.importsGrouped))) {
    ...
    *_installDataServices(pluginContext, urlBase) {
        const plugin = pluginContext.pluginDef;
        ...
        const pluginChain = [
            ...
            commonMiddleware.injectServiceHandles(serviceHandleMap, false),
        ];
    ...
    _resolveImports(plugin, urlBase) {
        ...
        for (const localName of Object.keys(plugin.importsGrouped)) {
            ...
            for (const version of Object.keys(group.versions)) {
                ...
                const importedRouter = this.routers[importedService.sourcePlugin][importedService.sourceName][importedService.version];
                if (!importedRouter) {
                    throw new Error(`Import ${importedService.sourcePlugin}:${importedService.sourceName}`
                        + " can't be satisfied");
                }
    ...
    _installSwaggerCatalog(plugin, urlBase) {
        ...
        this.pluginRouter.use(zLuxUrl.join(urlBase, '/catalogs/swagger'), router);
    }
    injectPluginRouter() {
