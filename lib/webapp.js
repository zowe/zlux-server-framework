...
const staticHandlers = {
  ...
  server(options){
    ...
    router.get('/', (req, res) => {
      res.send({
        "server": ['/server/config', '/server/log', '/server/logLevels', '/server/environment']
      });
    });
    ...
    router.get('/log', (req, res) => {
      if(process.env.LOG_PATH){
        res.sendFile(process.env.LOG_PATH);
      } else {
        es.send('Log not found');
      }
    });
    router.get('/logLevels', (req, res) => {
      res.send(global.COM_RS_COMMON_LOGGER.getConfig());
    });
    router.get('/environment', (req, res) => {
      ...
      res.send({
        ...
        "node.js release": process.release,
        "node.js config options": process.config,
      });
    });
    return router;
  },
  ...
  serverAgent(){
    ...
    router.get('/', (req, res) => {
      res.send({
        "/server/agent": ['/server/agent/config', '/server/agent/log', '/server/agent/logLevels', '/server/agent/environment']
      });
    });
    router.get('/config', (req, res) => {
      res.send('/config');
...
WebApp.prototype = {
  ...
  installRootServices() {
    ...
    this._installRootService('/server/agent', 'use', staticHandlers.serverAgent(), 
        {needJson: false, needAuth: false, isPseudoSso: false});
    serviceHandleMap['server'] = new WebServiceHandle('/server/agent', this.wsEnvironment);
    ...
    //this.expressApp.use(staticHandlers.server());
  },
