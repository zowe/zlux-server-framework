    ...
    if(!rbac){
      ...
      router.post('/config/httpsPort/:port', (req, res) => {
        ...
        if(isNaN(Number(req.params.port))){
          res.status(400).json({error: `Port query expected integer, received ${req.params.port}`});
        } else if(Number(req.params.port) < 0){
          res.status(400).json({error: `Port query expected positive integer, received ${req.params.port}`});
        }
        if(process.clusterManager){
          //should parse this.options object rather than config file
          ...
          fs.writeFileSync(options.configLocation, JSON.stringify(config, null, 2), (err) => {
            if(err){
              res.status(500).json({error: "Unable to update server configuration file"});
              return;
            }
          });
          process.clusterManager.setOverrideFileConfig(false);
          process.clusterManager.reloadAllWorkers();
          res.status(200).json({message: `Port remapped to ${req.params.port}`});
        } else {
        ...
        fs.access(fullPath,
          ...
          fs.stat(fullPath,
            ...
            fs.unlink(fullPath,(err)=> {
              ...
              res.status(200).json({message: "Deleting plugin '" + id + "'. Reloading server."});
              process.clusterManager.reloadAllWorkers();
            });
          });
        });
      ...
      if(process.clusterManager){
        ...
        fs.unlink(fullPath, (err) => {
          if(err && err.code == 'ENOENT'){
            //does not need full path
            res.status(400).json({error: `${fullPath} does not exist`});
            return;
          } else if(err && err.code == 'EACCES'){
            res.status(400).json({error: `Improper access permissions for ${fullPath}`});
            return;
          } else if(err){
        ...
        res.status(500).send('Server must be started in cluster mode to restart');
      }
    }
  },
  ...
  addOrUpgrade(webapp){
    return (req, res) => {
      if(process.clusterManager){
        ...
        if(pathToApp){
          //check out path.normalize
          pathToApp = pathToApp.replace(/\\/g, "/");
          if(!installApp.isFile(pathToApp)){
            ...
            if(installResponse.success === true){
              res.status(200).send(`Successfully installed ${installResponse.message}, reloading server`);
              process.clusterManager.reloadAllWorkers();
            } else {
          ...
          res.status(400).send('Name queries not yet supported');
        } else {
  ...
  addOrUpgradeStaticPlugin(webapp) {
    return (req, res)=> {
      if (process.clusterManager) {
        ...
        if (pathToApp) { // Attempt to extract a pluginDefinition given a path name & file
          try {
            //use path to maniuplate forward/back slashes
            pathToApp = pathToApp.split('\\').join('/'); // Replace '\' with '/'
            if (pathToApp[pathToApp.length - 1] == '/') {
              pathToApp = pathToApp.slice(0, pathToApp.length - 1); // **.filextension/ ---> **.filextension
            }

            const pluginDefFile = require(`${pathToApp}`) // Attempt to find specified file
            let fileName = pathToApp.split("/");
            ...
            res.status(404).json({message: "File location '" + pathToApp + "' doesn't exist."});
          }
