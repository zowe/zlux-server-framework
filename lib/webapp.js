...
function WebApp(options){
  ...
  const cookieIdentifier = options.serverConfig.cookieIdentifier;
...
WebApp.prototype = {
  ...
  installStaticHanders() {
    ...
    */
    ...
    const gatewayPort = this.options.serverConfig.node.mediationLayer.server.gatewayPort;
    const gatewayHost = this.options.serverConfig.node.mediationLayer.server.gatewayHost;
    ...
    if (this.options.gatewayRedirect) {
      this.expressApp.get('/', (req,res,next) => {
        ...
        } else if (this.options.rootRedirectURL) {
          res.redirect(this.options.gatewayRedirect+this.options.rootRedirectURL);
        } else {
          next();
        }
      });
      ...
      this.expressApp.get('/', function(req,res) {
        res.redirect('.'+this.options.rootRedirectURL);
      });
      this.expressApp.use('.'+this.options.rootRedirectURL, express.static(webdir));
    } else {
