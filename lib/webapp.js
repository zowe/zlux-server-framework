    ...
    default:
      ...
      for (let i = 0; i < langManagers.length; i++) {
        ...
        if (langManager.getSupportedTypes().includes(service.type)) {
          let connectionInfo = langManager.getConnectionInfo(plugin.identifier, service.type, service.name);
          if (connectionInfo) {
            //TODO resolve localhost to something better... allow binding of java to specific IPs like we did for node
            router = this.makeProxy(connectionInfo.url, false,
      ...
      if (!foundManager) {
        installLog.warn(`Could not load service ${plugin.identifier}:${service.name} `
