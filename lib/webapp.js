...
const staticHandlers = {
  ...
  server(options){
    ...
    if(!rbac){
      ...
      router.post('/config/httpsPort/:port', function(req, res){
        //take attributes instead
        if(isNaN(Number(req.params.port))){
          return res.status(400).json({error: `Port query expected integer, received ${req.params.port}`});
        } else if(Number(req.params.port) < 0){
          return res.status(400).json({error: `Port query expected positive integer, received ${req.params.port}`});
        }
        if(process.clusterManager){
          ...
          return res.status(200).json({message: `Port remapped to ${req.params.port}`});
        } else {
          return res.status(400).json({error: 'Server must be running in cluster mode to remap http(s) port'});
        }
      })
    ...
    return (req, res) =>{
      if(process.clusterManager){
        ...
        fs.unlink(fullPath, (err) => {
          ...
          // failed ZSS authentication issues. Error is uncertain, as increasing the timeout doesn't solve
          // the issue. Is probably a deeper sync that newer versions of Node handle in correct order.
          let nodeVer = process.version.substring(1, process.version.length);
          let split = nodeVer.split('.');
          if (split[0] <= 7) {
            process.clusterManager.reloadAllWorkers();
          }
        })
