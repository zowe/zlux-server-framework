  ...
  return new Promise(function(resolve, reject){
    ...
    if (rbac) {
      resolve({
        ...
        "agent": {
          "mediationLayer": nodeConfig.agent ? nodeConfig.agent.mediationLayer : undefined
        },
        "PID": process.pid,
        "PPID": process.ppid,
...
const staticHandlers = {
  ...
  server(options){
    ...
    const dataserviceAuth = options.componentConfig["dataserviceAuthentication"];
    ...
    if(!rbac){
      ...
      proxyOptions = Object.assign(proxyOptions, getAgentProxyOptions(options.componentConfig, options.tlsOptions, true));
...
function WebApp(options){
  ...
  options.componentConfig = options.componentConfig;
  ...
  this.loopbackConfig = {
    port: this.componentConfig.node.https?.port || this.componentConfig.node.http?.port,
    isHttps: this.componentConfig.node.https !== undefined,
    host: this.componentConfig.node.loopbackAddress
      ? this.componentConfig.node.loopbackAddress
      : zluxUtil.getLoopbackAddress(this.componentConfig.node.https?.ipAddresses || this.componentConfig.node.http?.ipAddresses)
  };
...
WebApp.prototype = {
  ...
  setValidReferrers() {
    if (this.options.componentConfig.checkReferrer.hosts) {
      ...
      let externalDomain = this.options.zoweConfig.externalDomains[0];
      ...
      const ipAddresses = (this.options.componentConfig.node.https?.ipAddresses || []).concat(this.options.componentConfig.node.http?.ipAddresses || []);
  ...
  installStaticHanders() {
    ...
    const rootPage = this.options.componentConfig.node.rootRedirectURL? '.'+this.options.componentConfig.node.rootRedirectURL
        : '/';
    if (rootPage != '/') {
    ...
    for (const proxiedRootService of this.options.componentConfig.agent.rootServices || []) {
      ...
      if (proxiedRootService.requiresAuth === false) {
        const _router = this.makeProxy(proxiedRootService.url, true,
                                       getAgentProxyOptions(this.options.componentConfig, this.options.tlsOptions, false));
