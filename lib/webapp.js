...
WebApp.prototype = {
  ...
  _makeRouter: async function (service, plugin, pluginContext, pluginChain) {
    ...
    case "router": {
        ...
        if (!service.routerFactory) {
          router = await service.nodeModule(dataserviceContext);
          ...
          router = await service.nodeModule[service.routerFactory](
              dataserviceContext);
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _installDataServices: async function(pluginContext, urlBase) {
    const plugin = pluginContext.pluginDef;
    ...
    for (const serviceName of Object.keys(plugin.dataServicesGrouped)) {
      ...
      for (const version of Object.keys(group.versions)) {
        const service = group.versions[version];
        const subUrl = urlBase + zLuxUrl.makeServiceSubURL(service);
        const router = await this._makeRouter(service, plugin, pluginContext, 
                                               pluginChain);
  ...
  async installPlugin(pluginContext) {
    const plugin = pluginContext.pluginDef;
...
// via samesite attribute of cookies, which all supported browsers support
      ...
      await this._installDataServices(pluginContext, urlBase);
