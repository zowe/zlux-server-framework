...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _installIframePlugin(plugin, urlBase) {
    if(plugin.webContent && plugin.webContent.framework === 'iframe' && plugin.webContent.destination>'') {
      ...
      iframeRouterWithMiddleware.push((req,res,next) => {
          installLog.debug(`Loading iframe app ${plugin.identifier} located at ${plugin.webContent.destination}`);
          next();
      });
      ...
      router.use((req, res, callback) => {
        ...
        installLog.info(`Loading remote iframe app ${plugin.identifier} located at ${remoteUrl}`);
