...
const staticHandlers = {
  plugins: function(webapp) {
    const respondToGetPlugins = function(req, res, webapp, type, plugins) {
      ...
      const pluginDefs = plugins.filter(p => !p.fake).map(p => p.exportTranslatedDef(acceptLanguage));
      let pluginId;
      let pluginLocation;
  ...
  server(options){
    ...
    const agentSwaggerBaseUrl = `${zLuxUrl.makePluginURL('zlux', zluxUtil.agentSwaggerPlugin.identifier)}/catalogs/swagger`;
    router.get('/swagger', function(req, res, next) {
      res.redirect(`${agentSwaggerBaseUrl}`);
    });
    router.get('/swagger/:api', function(req, res, next) {
  ...
  swagger(productCode) {
    const router = express.Router();
    const agentSwaggerPlugin = zluxUtil.agentSwaggerPlugin;
    const agentSwaggerBaseUrl = `${zLuxUrl.makePluginURL(productCode, agentSwaggerPlugin.identifier)}/catalogs/swagger`;
    const zluxSwaggerBaseUrl = `${zLuxUrl.makePluginURL(productCode, 'org.zowe.configjs')}/catalogs/swagger`;
    ...
    router.get(`/api-docs/${productCode}`, function(req, res, next) {
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _installSwaggerCatalog(plugin, urlBase, nodeContext, hostsContext) {
    plugin.getApiCatalog(this.options.productCode, nodeContext, hostsContext).then((openApi) => {
      const router = express.Router();
  ...
  installPlugin: Promise.coroutine(function*(pluginContext) {
    ...
    const hostsContext = pluginContext.server.config.hosts;
    ...
    try {
      ...
      this._installSwaggerCatalog(plugin, urlBase, nodeContext, hostsContext);
