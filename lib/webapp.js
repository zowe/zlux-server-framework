...
const staticHandlers = {
  ...
  server(options){
    ...
    const rbac = dataserviceAuth.hasOwnProperty("rbac");
    if(!rbac || rbac && dataserviceAuth.rbac == false){
      router.get('/*', (req, res) => {
        res.status(401).send("Unauthorized - RBAC is disabled.  Enable in zluxserver.json");
      })
      return router;
    } else if (rbac && dataserviceAuth.rbac == true){
      router.get('/', (req, res) => {
        res.status(200).json({
          "links": [
            {
      ...
      router.post('/logLevels/pattern/:componentPattern/level/:level', (req, res) => {
        if(isNaN(Number(req.params.level))){
          ...
          global.COM_RS_COMMON_LOGGER.setLogLevelForComponentPattern(req.params.componentPatter, Number(req.params.level));
          res.status(200).send(req.params);
        }
      });
