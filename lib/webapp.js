...
// Per-IP rate limit thresholds for unauthenticated root services to mitigate brute-force attacks
const UNAUTHENTICATED_ROOT_SERVICE_RATE_LIMIT_MAX_REQUESTS = 20;
const UNAUTHENTICATED_ROOT_SERVICE_RATE_LIMIT_WINDOW_MS = 60000; //1 minute
...
function getUserEnv(rbac, zoweConfig, agentHost){
  var date = new Date();
...
const staticHandlers = {
  ...
  server(options){
    ...
    router.get('/environment', function(req, res){
      getUserEnv(rbac, options.zoweConfig, options.proxiedHost).then(result => {
    ...
    if(!rbac){
      ...
      router.get('/log', function(req, res){
        const logFile = process.env.ZWED_NODE_LOG_FILE;
        // /dev/null (unix) and NUL (windows) are sentinels for disabled file logging
        if(logFile && logFile !== '/dev/null' && logFile !== 'NUL'){
          return res.sendFile(logFile, function(err){
            if(err && !res.headersSent){
              res.status(500).json({error: 'ZWED0124E - Log not found'});
            }
          });
        } else {
...
*/
  ...
  localCheck(loopbackSecret, localIp) {
    const expected = Buffer.from(loopbackSecret);
    return function localCheck(req, res, next) {
      ...
      const provided = loopbackData ? Buffer.from(loopbackData) : null;
      // timingSafeEqual() helps protect against guessing the value of the secret through inspection of timing of operations
      const secretsMatch = provided !== null && provided.length === expected.length && crypto.timingSafeEqual(expected, provided);
      if (secretsMatch && ((req.ip == localIp || address.range() == 'loopback') || req._zsfinternal)) {
        next();
      } else {
  ...
  /**
   * Per-IP rate limit for proxied root services that are reachable without an authenticated
   * session (requiresAuth:false, e.g. /login, /password). Each call to this function creates
   * its own independent limiter/store, so each such service is throttled separately.
   */
  rateLimitUnauthenticatedRootService(serviceName, maxRequests, windowMs) {
    return rateLimit({
      windowMs: windowMs,
      limit: maxRequests,
      ...
      handler: (req, res, next, options) => {
        routingLog.warn(`ZWED0306W`, req.ip, serviceName); //routingLog.warn(`Rate limit exceeded for IP=${req.ip} on unauthenticated root service=${serviceName}`);
        res.status(options.statusCode).json({error: `Too many requests to ${serviceName}, please try again later`});
      }
    });
  },
...
/*
  Returns the secret handed down by the cluster manager, when present, so that every
  worker within a cluster agrees on the same value. When no such value exists (for
  example when the server is not run under the cluster manager) an unpredictable
  value is generated for the lifetime of this process instead.

  Note: values read back from process.env are always strings, so a property that was
  assigned the value undefined reads back as the truthy string 'undefined'.
*/
function getSecretOrGenerate(envSecret) {
  if (envSecret && envSecret !== 'undefined') {
    return envSecret;
  }
...
function WebApp(options){
  ...
  if (!cookieSecret) {
    cookieSecret = getSecretOrGenerate(process.env.expressSessionSecret);
  }
  ...
  this.loopbackSecret = getSecretOrGenerate(process.env.loopbackSecret);
  delete process.env.expressSessionSecret;
  delete process.env.loopbackSecret;
  ...
  installRootServices() {
    ...
    for (const proxiedRootService of this.options.componentConfig.agent?.rootServices || []) {
      ...
      if (proxiedRootService.requiresAuth === false) {
        ...
        middlewareArray.push(commonMiddleware.rateLimitUnauthenticatedRootService(name,
            UNAUTHENTICATED_ROOT_SERVICE_RATE_LIMIT_MAX_REQUESTS, UNAUTHENTICATED_ROOT_SERVICE_RATE_LIMIT_WINDOW_MS));
