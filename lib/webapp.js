...
function DataserviceContext(serviceDefinition, serviceConfiguration, 
  ...
  setLoggerMessages(this.logger, 
    pluginContext.server.config.user.logLanguage,
    pluginContext.pluginDef.location);
}

function setLoggerMessages(logger, logLanguage, logLocation) {
  if (!logLanguage) {
    logLanguage = "en"; // Default to English if no language is specified
  }
  if (!logLocation) {
    bootstrapLogger.warn("Log location for logger '" + logger.componentName + "' is undefined")
    return;
  }
...
function makeSubloggerFromDefinitions(pluginDefinition, serviceDefinition, name) {
  console.log("MAKING SUBLOGGER HERE!!!");
