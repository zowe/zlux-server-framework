    ...
    default:
      ...
      for (let i = 0; i < langManagers.length; i++) {
        ...
        if (langManager.getSupportedTypes().includes(service.type)) {
          ...
          if (connectionInfo) {
            installLog.info(`ZWED0059I`, `${plugin.identifier}:${service.name}`, connectionInfo); //installLog.info(`Found connection info for ${plugin.identifier}:${service.name}=`,connectionInfo);
