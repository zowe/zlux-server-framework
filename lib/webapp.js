...
const staticHandlers = {
  ...
  healthAndInfo(healthRegistry) {
    const router = express.Router();
    router.get('/server/info', function(req, res, next) {
      res.json({
        id: 'zlux',
        version: zluxUtil.getZoweVersion(),
        zluxVersion: zluxUtil.getZluxVersion(),
        hostName: os.hostname()
      });
    });
    router.get('/server/health', function(req, res, next) {
      if (!healthRegistry) {
      ...
      for (const [name, entry] of Object.entries(healthRegistry)) {
        ...
        if (entry.details) {
          components[name].details = entry.details;
        }
        if (status !== 'UP') {
          allUp = false;
        }
      }
      const overallStatus = Object.keys(components).length === 0 ? 'UP' : (allUp ? 'UP' : 'DOWN');
      const body = { status: overallStatus, components };
      res.status(overallStatus === 'UP' ? 200 : 503).json(body);
    });
    return router;
  },
...
WebApp.prototype = {
  ...
  installRootServices() {
    ...
    this.expressApp.use(staticHandlers.healthAndInfo(this.options.healthRegistry));
