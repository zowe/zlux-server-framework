...
const RaftService = require('./raft-rpc-ws').RaftRPCWebSocketService;
...
function DataserviceContext(serviceDefinition, serviceConfiguration, 
  ...
  if (process.clusterManager) { // Cluster mode
    ...
    this.storage = createLocalDataserviceStorage(webapp.storage, this.plugin.pluginDef.identifier);
  }
...
function createLocalDataserviceStorage(storage, pluginId) {
  return {
    getAll: function () {
      return Promise.resolve(storage.getAll(pluginId));
    },

    get: function (key) {
      return Promise.resolve(storage.get(key, pluginId));
    },

    setAll: function (dict) {
      storage.setAll(dict, pluginId);
      sync.setAllStorageForPlugin(pluginId, dict);
      return Promise.resolve();
    },
    
    set: function (key, value) {
      storage.set(key, value, pluginId);
      sync.setStorageForPlugin(pluginId, key, value);
      return Promise.resolve();
    },
    
    delete: function (key) {
      storage.delete(key, pluginId);
      sync.deleteStorageForPlugin(pluginId, key);
      return Promise.resolve();
    },
    
    deleteAll: function () {
      storage.setAll({}, pluginId);
      sync.setAllStorageForPlugin(pluginId, {});
      return Promise.resolve();
    }
  };
}
...
const staticHandlers = {
  ...
  raft(webApp) {
    const router = express.Router();
    webApp.expressWs.applyTo(router);
    router.ws('/raft', (ws, req) => {
      const service = new RaftService(ws, req, raft);
    });
    return router;
  },
...
function WebApp(options){
  ...
  this.expressApp.use(raft.middleware());
...
WebApp.prototype = {
  ...
  installRootServices() {
    ...
    this.expressApp.use(staticHandlers.raft(this));
  },
