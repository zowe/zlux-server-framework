...
WebApp.prototype = {
  ...
  makeProxy(urlPrefix, noAuth, overrideOptions, host, port) {
    if (!overrideOptions.tlsOptions) {
      console.trace(`makeProxy with no auth for urlPrefix ${urlPrefix} tlsOptions %s`, overrideOptions.tlsOptions ? "ok" : "not provided")
    }
    const r = express.Router();
    let proxiedHost;
    let proxiedPort;
  ...
  installRootServices() {
    ...
    for (const proxiedRootService of this.options.rootServices || []) {
      ...
      if (!this.options.tlsOptions) {
        console.trace('no tls options before making proxies');
      }
...
module.exports.makeWebApp = function (options) {
  if (options.tlsOptions) {
    console.log(`makeWebApp options provided}`);
  } else {
    console.log(`makeWebApp tlsOptions not provided :-(`);
  }
