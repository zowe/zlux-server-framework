...
const commonMiddleware = {
  ...
  setCsp(custom) {
    return function cspHeader(req, res, next) {
      let subdomains = req.subdomains;
      let length = req.subdomains.length-1;
      for (let i = 0; i < subdomains.length; i++) {
        length += subdomains[i].length;
      }
      let domain = req.hostname.substr(length);
      if (custom) {
        res.set('Content-Security-Policy', custom.replace(/\$DOMAIN/g, domain));
      } else {
        res.set('Content-Security-Policy', `default-src 'self' ${req.hostname};`);
      }
      next();
    }
  },
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _installPluginStaticHandlers(plugin, urlBase) {
    ...
    if (plugin.webContent && plugin.location) {
      ...
      if (plugin.webContent.csp == 'strict') {
        middleware.push(commonMiddleware.setCsp());
      } else if (plugin.webContent.cspString) {
        middleware.push(commonMiddleware.setCsp(plugin.webContent.cspString));
      }
