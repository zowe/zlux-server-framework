 ...
 */
function WebServiceHandle(urlPrefix, environment, isProxy) {
...
WebServiceHandle.prototype = {
  ...
  call(path, options, originalRequest, originalRes) {
    return new Promise((resolve, reject) => {
      ...
      let httpOrHttps;
      if (this.isProxy) {
        if(this.isProxy.type === 'external') {
         requestOptions.hostname = this.isProxy.host;
         requestOptions.port = this.isProxy.port;
         requestOptions.path = this.isProxy.urlPrefix;
         requestOptions.protocol = this.isProxy.isHttps ? 'https:': 'http:';
        } else {
  ...
  _makeServiceHandleMap(plugin, urlBase) {
    ...
    for (const group of zluxUtil.concatIterables(
      ...
      for (const version of Object.keys(group.versions)) {
        ...
        const isProxy = (service.type === 'service' || service.type === 'external') ? service: false;
        const handle = new WebServiceHandle(subUrl, this.wsEnvironment, isProxy);
