...
const staticHandlers = {
  plugins: function(webapp) {
    const respondToGetPlugins = function(req, res, webapp, type, plugins) {
      ...
      const pluginDefs = plugins.filter(p => p.identifier!='org.zowe.zss.swagger').map(p => p.exportTranslatedDef(acceptLanguage));
      let pluginId;
      let pluginLocation;
  ...
  swagger() {
    const router = express.Router();
    router.get('/api-docs/zss', function(req, res, next) {
      res.redirect('/ZLUX/plugins/org.zowe.zss.swagger/catalogs/swagger');
    });

    router.get('/api-docs/zlux', function(req, res, next) {
      res.redirect('/ZLUX/plugins/org.zowe.configjs/catalogs/swagger');
    });

    router.get('/api-docs/zss/:api', function(req, res, next) {
      res.redirect(`/ZLUX/plugins/org.zowe.zss.swagger/catalogs/swagger/${req.params.api || ''}`);
    });
    return router;
  },
...
WebApp.prototype = {
  ...
  installRootServices() {
    ...
    this.expressApp.use(staticHandlers.swagger());
  },
