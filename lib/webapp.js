      ...
      router.post('/config/:attribute/', function(req, res){
        ...
        if(allowedPropertyValue !== undefined){
          ...
          if(body[lastProperty] == undefined){
            return res.status(400).json({error: "Response body property name does not match query property name"});
          }
          if(allowedPropertyValue === (typeof body[lastProperty])){
            ...
            return res.status(400).json({error: `Request body of type ${(typeof body[attribParts[attribParts.length - 1]])} does not match expected value of ${_.get(allowed, attrib)}`});
          }
        } else {
