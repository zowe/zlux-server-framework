...
function createDataserviceStorage(pluginId) {
  ...
  return process.clusterManager.getStorageAll(pluginId).then(function (clusterStorage) {
    ...
    storageObj.deleteAll = function (key) {
      sync.deleteAllStorageForPlugin(pluginId);
...
WebApp.prototype = {
  ...
  installRootServices() {
    ...
    this.expressApp.use(staticHandlers.sync(this));
  },
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  installSyncClient() {
    const port = this.options.serverConfig.node.https.port;
    console.log(`installSyncClient port ${port}`);
    if (port !== 8544) {
      console.log('installSyncClient backup');
      sync.setAsBackup();
      const client = new SyncClient();
    } else {
      console.log('installSyncClient master');
      sync.setAsMaster();
    }
  },
