...
function DataserviceContext(serviceDefinition, serviceConfiguration, 
  ...
  createDataserviceStorage().then((data) => {
    ...
    let localStorage = this.storage.getStorage();
    localStorage = { ...localStorage, ...data.storageDict }
    data.setStorage(localStorage);
    this.storage = data;
  });
...
function createDataserviceStorage() {
  if (process.clusterManager ) { // Cluster mode

    return process.clusterManager.getStorage().then( function(clusterStorage) {

      clusterStorage.getStorage = function () {
        return process.clusterManager.getStorage().then((storage) => {
          return storage.storageDict;
        });
      }

      clusterStorage.setStorageValue = function (id, value) {
        clusterStorage.storageDict[id] = value;
        process.clusterManager.setStorageValue(id, value);
      }

      clusterStorage.setStorage = function (dict) {
        clusterStorage.storageDict = dict;
        process.clusterManager.setStorage(dict);
      }

      clusterStorage.getStorageValue = function (id) {
        process.clusterManager.getStorageValue(id).then((value) => {
          return value;
        });
      }

      clusterStorage.deleteStorageValue = function (id) {
        delete clusterStorage.storageDict[id];
        process.clusterManager.deleteStorageValue(id);
      }

      console.log("App context is loaded with storage object: ", clusterStorage);
      return clusterStorage;
    })
    .catch( function(e) { // Chances are, if storage failed to load, then the logger may have failed to load
      console.error("App context will be loaded with default storage, error encountered: ", e);
      return zluxUtil.DataserviceStorage();
    })
  } else { // Non-cluster mode
    console.log("App context is loaded with storage object2: ", zluxUtil.DataserviceStorage());
    // Non-cluster mode means we don't need to compare storage between clusters, so we can maintain 1 object
    return zluxUtil.DataserviceStorage();
  }
}
  ...
  _installPluginStaticHandlers(plugin, urlBase) {
    ...
    if (plugin.pluginType === "library") {
      if (plugin.libraryVersion) {
        ...
        installLog.warn(`Library ${plugin.identifier} is missing libraryVersion attribute for hosting files. `
