  ...
  call(path, options, originalRequest, originalRes) {
    return new Promise((resolve, reject) => {
      ...
      if (this.isAgentService) {
        if(this.isAgentService.type === 'external') {
         requestOptions.hostname = this.isAgentService.host;
         requestOptions.port = this.isAgentService.port;
         requestOptions.path = this.isAgentService.urlPrefix;
         requestOptions.protocol = this.isAgentService.isHttps ? 'https:': 'http:';
        } else {
  ...
  _makeServiceHandleMap(plugin, urlBase) {
    ...
    for (const group of zluxUtil.concatIterables(
      ...
      for (const version of Object.keys(group.versions)) {
        ...
        const isAgentService = (service.type === 'service' || service.type === 'external') ? service: false;
        const handle = new WebServiceHandle(subUrl, this.wsEnvironment, isAgentService);
