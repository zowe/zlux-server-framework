...
WebServiceHandle.prototype = {
  ...
  call(path, options, originalRequest, originalRes) {
    return new Promise((resolve, reject) => {
      ...
      while(this.service && this.service.type === 'import') {
        let reqDataservice;
        const reqPlugin = pluginsArray.filter(plugin => plugin.identifier === this.service.sourcePlugin)
        reqPlugin.filter(plugin => {
          const dataservices = plugin.dataServices.filter(dataService => dataService.name === this.service.sourceName)
          reqDataservice = dataservices;
        })
        this.service = reqDataservice[0];
      }
