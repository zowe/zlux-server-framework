...
WebApp.prototype = {
  ...
  _makeRouter: function *(service, plugin, pluginContext, pluginChain) {
    ...
    case "java-war":
    case "java-jar":
      let javaManager = this.options.langManagers.javaManager;
      if (javaManager) {
        let connectionInfo = javaManager.getConnectionInfo(plugin.identifier, service.type, service.name);
        if (connectionInfo) {
          //TODO resolve localhost to something better... allow binding of java to specific IPs like we did for node
          router = this.makeProxy(connectionInfo.url, false, connectionInfo.options, 'localhost', connectionInfo.port);
        } else {
          throw new Error(`Could not resolve java service URL. Plugin=${plugin.identifier}, service=${service.name}`);
        }
      } else {
        throw new Error(`Could not load java service. No java manager found`);
      }
      break;
    case "nodeService":
      //installLog.info(
