    ...
    if (rbac) {
      ...
      resolve({
        ...
        "userEnvironment": {
          ...
          "GATEWAY_PORT": process.env.ZWE_components_gateway_port,
        },
        "agent": {
          "mediationLayer": serverConfig.agent ? serverConfig.agent.mediationLayer : undefined
        }
...
const staticHandlers = {
  ...
  server(options){
    ...
    const zluxSwaggerBaseUrl = `${zLuxUrl.makePluginURL(options.serverConfig.node.productCode, zluxUtil.serverSwaggerPluginId)}/catalogs/swagger`;
    ...
    if(!rbac){
      ...
      let proxyOptions = {
        ...
        allowInvalidTLSProxy: options.serverConfig.node.allowInvalidTLSProxy
      };
...
function WebApp(options){
  options.serverConfig = options.zoweConfig.components['app-server'];
  this.expressApp = expressApp;
  const port = options.httpsPort ? options.httpsPort : options.httpPort;
  const cookieIdentifier = options.serverConfig.zowe.cookieIdentifier;
  ...
  if (typeof options.serverConfig.readBodyLimit == 'number') { 
    DEFAULT_READBODY_LIMIT = options.serverConfig.readBodyLimit;
  }
...
WebApp.prototype = {
  ...
  setValidReferrers(options) {
    if (this.options.serverConfig.checkReferrer.hosts) {
      this.validDomains = this.options.serverConfig.checkReferrer.hosts;
    } else {
  ...
  toString() {
    return `[WebApp product: ${this.options.serverConfig.node.productCode}]`
  },
  ...
  makeProxy(urlPrefix, noAuth, overrideOptions, host, port) {
    ...
    let options = {
      ...
      allowInvalidTLSProxy: this.options.serverConfig.node.allowInvalidTLSProxy,
  ...
  installStaticHanders() {
    const webdir = path.join(path.join(this.options.serverConfig.productDir,
      this.options.serverConfig.node.productCode), 'web');
    const rootPage = this.options.serverConfig.node.rootRedirectURL? '.'+this.options.serverConfig.node.rootRedirectURL
        : '/';
    if (rootPage != '/') {
  ...
  installCommonMiddleware() {
    this.expressApp.use(commonMiddleware.addAppSpecificDataToRequest(
      this.appData, this.loopbackSecret, this.options.serverConfig.node.internalRouting));
  },
  ...
  installRootServices() {
    ...
    for (const proxiedRootService of this.options.serverConfig.agent.rootServices || []) {
    ...
    this.expressApp.use(staticHandlers.swagger(this.options.serverConfig.node.productCode));
  },
  ...
  _makeRouterForLegacyService(pluginContext, service) {
    ...
    const router = express.Router();
    const urlSpec = "/" + this.options.serverConfig.node.productCode + "/plugins/" 
    ...
    const manager = {
      ...
      productCode:this.options.serverConfig.node.productCode
    };
  ...
  _makeRouter: function *(service, plugin, pluginContext, pluginChain) {
    ...
    case "service":
      ...
      router = this.makeProxy(service.urlPrefix
        ? service.urlPrefix
        : zLuxUrl.makePluginURL(this.options.serverConfig.node.productCode, plugin.identifier)
    ...
    case "router": {
        ...
        const serviceConfiguration = configService.getServiceConfiguration(
            plugin.identifier,  service.name, 
            pluginContext.server.config.app, this.options.serverConfig.node.productCode);
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _installSwaggerCatalog(plugin, urlBase, nodeContext) {
    ...
    zluxUtil.timeout(ms)
    .then(() => plugin.getApiCatalog(this.options.serverConfig.node.productCode, nodeContext)).then((openApi) => {
      const router = express.Router();
  ...
  installPlugin: Promise.coroutine(function*(pluginContext) {
    const plugin = pluginContext.pluginDef;
    const urlBase = zLuxUrl.makePluginURL(this.options.serverConfig.node.productCode, 
        plugin.identifier);
    const nodeContext = this.options.serverConfig.node;
...
// via samesite attribute of cookies, which all supported browsers support
      if (this.options.serverConfig.node.checkReferrer?.enabled) {
    ...
    this.expressApp.use((req, res, next) => {
      ...
      for (const header of Object.keys(headers)) {
        ...
        if (header == 'referer') {
          ...
          var pattern = new RegExp('^http.+\/'+this.options.serverConfig.node.productCode+'\/plugins\/.+');
          if (pattern.test(referrer)) {
            const parts = headers[header].split("/");
            const zluxIndex = parts.indexOf(this.options.serverConfig.node.productCode);
            ...
            if (myProxy != undefined) {
              ...
              return do404(req.url, res, this.options.serverConfig.node.productCode
            ...
            return do404(req.url, res, this.options.serverConfig.node.productCode + ": unknown resource requested");
          }
          break;
        }
      }
