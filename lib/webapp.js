...
/* Set of "<pluginID>:<serviceName>" for every external-type dataservice that has
 * been installed. Used only to decide whether the referrer fall-through in
 * installErrorHanders() may re-dispatch a request. It deliberately holds no
 * request handler: the fall-through must re-enter the service's full middleware
 * chain (auth included) rather than call a bare proxy. */
...
WebApp.prototype = {
  ...
  /* Parses a referrer and returns it as a URL object if it points at one of this
   * server's own hostnames, else null. allowInsecure additionally accepts http://,
   * for servers not listening on https; checkReferrer() intentionally does not pass
   * it, keeping its original https-only behavior.
   *
   * The host is compared after real URL parsing rather than by string prefix, so
   * that "https://valid.host:pw@malicious.com/" (where the valid domain sits in the
   * userinfo and the actual host is malicious.com) does not pass, and so that an exact
   * origin with no port or trailing slash ("https://valid.host") does. */
  parseOwnDomainReferrer(ref, allowInsecure) {
    if (!ref || !Array.isArray(this.validDomains)) {
      return null;
    }
    let parsed;
    try {
      parsed = new url.URL(ref);
    } catch (e) {
    ...
    const hostname = parsed.hostname.toLowerCase(); //no port, no userinfo
    const host = parsed.host.toLowerCase(); //includes :port when present
    for (let i = 0; i < this.validDomains.length; i++) {
      const domain = String(this.validDomains[i]).toLowerCase();
      /* hostname covers the normal host-only entry, host covers a configured
       * checkReferrer.hosts entry that included a port, and the bracketed form
       * covers IPv6 listening addresses, which URL reports as "[::1]". */
      if (hostname === domain || host === domain || hostname === '['+domain+']') {
        return parsed;
      }
    }
    return null;
  ...
  isOwnDomainReferrer(ref, allowInsecure) {
    return this.parseOwnDomainReferrer(ref, allowInsecure) !== null;
  },
  ...
  checkReferrer(req, res, next) {
    ...
    if (this.isOwnDomainReferrer(ref)) {
      return next();
    }
  ...
  makeExternalProxy(host, port, urlPrefix, isHttps, noAuth, pluginID, serviceName) {
    ...
    externalServices.add(pluginID + ":" + serviceName);
    r.use(myProxy);
    return r;
  },
  ...
  _installRootService(url, method, handler, {needJson, needAuth, authType, isPseudoSso}) {
    ...
    installLog.debug(`ZWED0054I`, url); //installLog.info(`installing root service at ${url}`);
  ...
  installRootServices() {
    ...
    for (const proxiedRootService of this.options.componentConfig.agent?.rootServices || []) {
      ...
      installLog.debug(`ZWED0055I`, proxiedRootService.url); //installLog.info(`installing root service proxy at ${proxiedRootService.url}`);
...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
    ...
    for (const serviceName of Object.keys(plugin.dataServicesGrouped)) {
      ...
      for (const version of Object.keys(group.versions)) {
        ...
        const router = yield* this._makeRouter(service, plugin, pluginContext, 
                                               pluginChain);
        installLog.debug(`ZWED0062I`, plugin.identifier, subUrl); //installLog.info(`${plugin.identifier}: installing router at ${subUrl}`);
  ...
  _resolveImports(plugin, urlBase) {
    ...
    for (const localName of Object.keys(plugin.importsGrouped)) {
      ...
      for (const version of Object.keys(group.versions)) {
        ...
        installLog.debug(`ZWED0064I`, plugin.identifier, importedService.sourcePlugin, importedService.sourceName, subUrl); //installLog.info(`${plugin.identifier}: installing import` + ` ${importedService.sourcePlugin}:${importedService.sourceName}` + ` at ${subUrl}`);
  ...
  _installPluginStaticHandlers(plugin, urlBase) {
    ...
    if (plugin.webContent && plugin.location) {
      let url = `${urlBase}/web`;
      installLog.debug(`ZWED0066I`, plugin.identifier, url); //installLog.info(`${plugin.identifier}: serving static files at ${url}`);
    ...
    if (plugin.pluginType === "library") {
      if (plugin.libraryVersion) {
        ...
        installLog.debug(`ZWED0067I`, plugin.identifier, url); //installLog.info(`${plugin.identifier}: serving library files at ${url}`);
  ...
  _installSwaggerCatalog(plugin, urlBase) {
    ...
    .then(() => plugin.getApiCatalog(this.options.zoweConfig)).then((openApi) => {
      const router = express.Router();
      installLog.debug(`ZWED0068I`, plugin.identifier); //installLog.info(`Creating composite swagger endpoint for ${plugin.identifier}`);
...
// via samesite attribute of cookies, which all supported browsers support
  ...
  installErrorHanders() {
    const productCode = this.options.componentConfig.node.productCode;
    /* A request that matched no route may still belong to an external-type
     * dataservice whose backend emitted root-relative URLs. The referrer tells us
     * which service that was, but it is client-controlled, so it is only ever used
     * to *look up* an already-installed service - never to build a destination.
     * The request is then re-dispatched through the plugin router so that the
     * service's own middleware chain (checkReferrer, injectServiceDef,
     * auth.middleware, localCheck for internalOnly, no-cache, logging) runs.
     */
    /* Matched against the parsed pathname, not the raw header, so that this and the
     * host check cannot disagree about where the authority ends. URL parsing has
     * already resolved any ".." segments by this point. */
    const referrerPathPattern = new RegExp('^\/'+escapeRegExp(productCode)
        +'\/plugins\/([^\/]+)\/services\/([^\/]+)\/([^\/]+)');

    this.expressApp.use((req, res, next) => {
      const originalUrl = req.originalUrl;
      const notFound = () => do404(originalUrl, res, productCode + ": unknown resource requested");
      const referrer = req.get('referer');
      if (!referrer) {
        return notFound();
      }
      /* Reject referrers naming any host other than our own. Without this, a
       * forged header alone reaches a plugin's proxy. */
      const parsedReferrer = this.parseOwnDomainReferrer(referrer, !this.options.isHttps);
      if (!parsedReferrer) {
        routingLog.warn('ZWED0172W', referrer, req.originalUrl, req.ip);
        return notFound();
      }
      const match = referrerPathPattern.exec(parsedReferrer.pathname);
      if (!match) {
      ...
      /* Resolve against what is actually installed. Only external-type services
       * ever used this path, so keep it that way. */
      if (!externalServices.has(pluginID + ":" + serviceName)
          || !(this.routers[pluginID] && this.routers[pluginID][serviceName]
               && this.routers[pluginID][serviceName][version])) {
      ...
      const serviceUrl = zLuxUrl.makePluginURL(productCode, pluginID)
          + '/services/' + serviceName + '/' + version;
      utilLog.debug("ZWED0306I", serviceUrl, originalUrl);
      req.url = serviceUrl + originalUrl;
      this.pluginRouter(req, res, (err) => {
        req.url = originalUrl;
        if (err) {
          return next(err);
        }
