  ...
  call(path, options, originalRequest, originalRes) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(this.service && this.service.imported) {
        const reqPath = requestOptions.path.split('/').slice(6).join('/');
        requestOptions.path = `/ZLUX/plugins/${this.service.sourcePlugin}/services/${this.service.sourceName}/${reqPath}`
      ...
      if (httpOrHttps) {
        ...
        if(requestOptions.hostname === this.environment.agentRequestOptions.host && requestOptions.port === this.environment.agentRequestOptions.port) {
          ...
          const reqPath = requestOptions.path;
          const pathArray = reqPath.split('/');
          const reqPathVersion = pathArray.length >= 7 ? pathArray[6] : '';
          if(versionPattern.test(reqPathVersion)) {
            requestOptions.path = reqPath.replace('/'+reqPathVersion, '');
          }
