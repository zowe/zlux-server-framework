...
function getAttrib(object, path){
  if(object === undefined || typeof object !== 'object') return undefined;
...
function setAttrib(object, path, value){
  if(object === undefined || typeof object !== 'object') return undefined;
  if(value === undefined || path === undefined) return object;
  let props = path.split('.');
  let newObj = Object.assign({}, object);
  for(let i = 0; i < props.length; i++){
    if(i == props.length - 1){
      if(newObj[props[i]] !== undefined){
        newObj[props[i]] = value;
        break;
      }
    }
  ...
  server(options){
    ...
    if(!rbac){
      ...
      router.post('/config/:attribute/', function(req, res){
        ...
        let allowedPropertyValue = getAttrib(allowed, attrib);
        if(allowedPropertyValue !== undefined){
          if(allowedPropertyValue === (typeof body[lastProperty])){
            ...
            return res.status(200).json({
              ...
              expectedType: allowedPropertyValue,
