...
//      installLog.info(`${plugin.identifier}: installing external proxy at ${subUrl}`);
  ...
  _checkPluginRequirements(plugin) {
    if (plugin.pluginReqs) {
      const components = plugin.pluginReqs.components;
      const capabilities = plugin.pluginReqs.capabilities;
      if (components) {
        if (components['org.zowe.core.zlux.zlux-core']) {
          
      console.log("One boy has requirements!", components['org.zowe.core.zlux.zlux-core'].version);
        }
      }
      if (capabilities) {
        // TODO: Declare capaibilities
      }
    }
    //throw new Error('Unable to verify the environment!');
  },
  ...
  installPlugin: Promise.coroutine(function*(pluginContext) {
    ...
    console.log("Before each plugin, this should appear!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", plugin);

    try {
      this._checkPluginRequirements(plugin);
