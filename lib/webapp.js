...
WebApp.prototype = {
  ...
  _makeRouter: function *(service, plugin, pluginContext, pluginChain) {
    ...
    if (service.internalOnly) {
      serviceRouterWithMiddleware.push(commonMiddleware.localCheck(this.loopbackSecret));
    }
    let router;
    switch (service.type) {
    case "service":
