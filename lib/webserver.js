...
function readTlsOptionsFromConfig(config, httpsOptions) {
  ...
  if (config.https.pfx) {
    ...
    if (config.https.certificates) {
      ...
      for(let i = 0; i < httpsOptions.cert.length; i++){
        ...
        console.log(`Certificate ${i}: `, 'Signature: ' + curCert.signature, 
                    'Validity: ' + curCert.validity, 'Issuer: ' + curCert.issuer, 
                    'Extensions: ' + curCert.extensions, 'Public key: ' + curCert.publicKey, 'Cipher: ' + curCert.md);
      }
  ...
  if (config.https.certificateAuthorities) {
    ...
    for(let i = 0; i < httpsOptions.ca.length; i++){
      ...
      console.log(`CA Certificate ${i}: `, 'Signature: ' + curCert.signature, 
                  'Validity: ' + curCert.validity, 'Issuer: ' + curCert.issuer, 
                  'Extensions: ' + curCert.extensions, 'Public key: ' + curCert.publicKey, 'Cipher: ' + curCert.md);
    }
  }
