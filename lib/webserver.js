...
function makeWebServerHttpAndHttpsOptions(wsConfig) {
  let httpsOptions;
  let httpOptions;
  if (wsConfig.http && wsConfig.http.port) {
    httpOptions = {};
  }
  if (wsConfig.https && wsConfig.https.port) {
    let options = wsConfig.https;
    httpsOptions = {};
    if (typeof wsConfig.allowInvalidTLSProxy == 'boolean') {
      httpsOptions.rejectUnauthorized = !wsConfig.allowInvalidTLSProxy;
    }
    ...
    if (typeof options.secureOptions == 'number') {
      ...
      httpsOptions.secureOptions = options.secureOptions;
    } else if (typeof options.secureProtocol == 'string') {
      httpsOptions.secureProtocol = options.secureProtocol;
    } else {
      ...
      httpsOptions.secureOptions = consts.SSL_OP_NO_SSLv2 |
    ...
    readTlsOptionsFromConfig(wsConfig, httpsOptions);
    return {httpOptions, httpsOptions};
  }
}
...
WebServer.prototype = {
  ...
  setConfig(config) {
    const {httpOptions, httpsOptions} = makeWebServerHttpAndHttpsOptions(config);
    this.config = config;
...
module.exports.makeWebServerHttpAndHttpsOptions = makeWebServerHttpAndHttpsOptions;
