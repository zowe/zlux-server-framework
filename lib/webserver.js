...
function loadPem(locations, type, keyrings, pass) {
  ...
  if (files) {
    content = util.readFilesToArray(files, type, pass).concat(content);
  }
...
WebServer.prototype = {
  ...
  validateAndPreprocessConfig: Promise.coroutine(function *validateAndPreprocessConfig(zoweConfig) {
    ...
    if (config.https?.port) {
      ...
      if(keyring_js && zoweConfig.zowe?.certificate?.truststore?.type == 'JCERACFKS') {
        const {keyringOwner, keyringName, label} = parseSafKeyringAddress(zoweConfig.zowe.certificate.truststore.file);
        let certificateList;
