...
WebServer.prototype = {
  ...
  validateAndPreprocessConfig: Promise.coroutine(function *validateAndPreprocessConfig(config) {
    ...
    if (config.https && config.https.port) {
      ...
      //TODO all a big hack. Migrate this to just read zowe.yaml content
      if(keyring_js && process.env.ZWE_zowe_certificate_truststore_type == 'JCERACFKS') {
        //shim the value into our nice parser by adding &dummy at the end...
        const {owner, ringName, label} = parseSafKeyringAddress(process.env.ZWE_zowe_certificate_truststore_file + '&dummy');
        let certificateList;
        if (!config.https.certificateAuthorities) {
        ...
        if(certificateList) {
          for(let i = 0; i < certificateList.length; i++) {
            if(certificateList[i].usage === 'CERTAUTH') {
              let safKeyring = `${process.env.ZWE_zowe_certificate_truststore_file}&${certificateList[i].label}`;
