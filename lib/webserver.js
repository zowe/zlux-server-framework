...
function readCiphers(stringOrArray) {
  ...
  if (typeof stringOrArray == 'string') {
    ...
    cipherArray = stringOrArray.split(':');
  }
  if (cipherArray && cipherArray.length > 0) {
    let uppercase = [];
    for (let i = 0; i < cipherArray.length; i++) {
      if (typeof cipherArray[i] != 'string') {
        bootstrapLogger.warn(`ZWED0069W`, cipherArray[i]); //bootstrapLogger.warn(`Returning null for cipher array because input had non-string: `,cipherArray[i]);
        return null;
      }
      uppercase[i] = cipherArray[i].toUpperCase();
      //likely to be iana if starts with TLS_ or SSL_CK
      //NOTE: there are some cases where openssl also prefixes with TLS_ but that seems to be
      //      when the iana and openssl names are identical (tls v1.3)
      if (uppercase[i].startsWith("TLS_") || uppercase[i].startsWith("SSL_CK")) {
        const mapping = constants.OPENSSL_CIPHER_NAME_FROM_IANA[uppercase[i]];
        if (mapping) { uppercase[i] = mapping; }
      }
    }
...
function loadPkcs12(location, pass) {
  let alias = process.env.ZWE_zowe_certificate_keystore_alias;
  ...
  locations.forEach((location)=> {
    p12.safeContents.forEach(safeContent => {
      safeContent.safeBags.forEach(content => {
       if (type == CRYPTO_CONTENT_CERT && content.type === forge.pki.oids.certBag) {
         if (content.attributes.friendlyName == alias) {
           value.push(forge.pki.certificateToPem(content.cert));
           return value;
         }
       } else if (type == CRYPTO_CONTENT_KEY && content.type === forge.pki.oids.keyBag) {
          if (content.attributes.friendlyName == alias) {
            value.push(pki.privateKeyToPem(content.key));
            return value;
          }
       } else if (type == CRYPTO_CONTENT_KEY && content.type === forge.pki.oids.pkcs8ShroudedKeyBag) {
          if (content.attributes.friendlyName == alias) {
            value.push(pki.privateKeyToPem(content.key));
            return value;
          }
       } else if (type == CRYPTO_CONTENT_CA && content.type === forge.pki.oids.certBag && content.cert?.extensions) {
         let isCa = false;
         for (let i = 0; i < content.cert.extensions.length; i++) {
           let extension = content.cert.extensions[i];
           //2.5.29.19
           if (extension.type == forge.pki.oids.basicConstraints) {
             isCa = extension.cA;
             break;
           }
            /*
            // 2.5.29.15 has 'keyCertSign: true' on a CA, i guess?
            if (extension.type == forge.pki.oids.keyUsage) {
            isCa = extension.keyCertSign;
            }
            */
         }
         if (isCa) {
           value.push(content.cert);
         }
       } else if (type == CRYPTO_CONTENT_CRL && content.type === forge.pki.oids.crlBag) {
         bootstrapLogger.warn(`Certificate Revocation List requested and found, but unhandled`);
         return [];
       } else {
         bootstrapLogger.debug(`Ignoring PKCS12 object type=${content.type}`);
       }
     });
   });
  });
...
function readTlsOptionsFromConfig(nodeConfig, httpsOptions, pass, alias) {
  ...
  if (nodeConfig.https.certificates) {
    httpsOptions.cert = loadPem(nodeConfig.https.certificates, CRYPTO_CONTENT_CERT, keyringCache, p12Cache,  pass, alias).content;
  ...
  if (nodeConfig.https.keys) {
    httpsOptions.key = loadPem(nodeConfig.https.keys, CRYPTO_CONTENT_KEY, keyringCache, p12Cache, pass, alias).content;
  }
  ...
  if (nodeConfig.https.certificateAuthorities) {
    httpsOptions.ca = loadPem(nodeConfig.https.certificateAuthorities, CRYPTO_CONTENT_CA, keyringCache, p12Cache, pass).content;
  }
  if (nodeConfig.https.certificateRevocationLists) {
    httpsOptions.crl = loadPem(nodeConfig.https.certificateRevocationLists, CRYPTO_CONTENT_CRL, keyringCache, p12Cache, pass).content;
  }
}
    ...
    if (typeof nodeConfig.https == 'object') {
      ...
      if (typeof options.secureOptions == 'number') {
        ...
        if (minTls <= constants.TLS_VERSION["TLSv1.2"] && maxTls >= constants.TLS_VERSION["TLSv1.2"]) {
          ...
          bootstrapLogger.debug("TLS 1.2 off");
          this.httpsOptions.secureOptions = this.httpsOptions.secureOptions | consts.SSL_OP_NO_TLSv1_2;
        }
        if (minTls <= constants.TLS_VERSION["TLSv1.3"] && maxTls >= constants.TLS_VERSION["TLSv1.3"]) {
          bootstrapLogger.debug("TLS 1.3 on");
        } else {
          bootstrapLogger.debug("TLS 1.3 off");
          this.httpsOptions.secureOptions = this.httpsOptions.secureOptions | consts.SSL_OP_NO_TLSv1_3;
        }
      }
      
      if (options.ciphers != 'NODEJS') {//for using nodejs defaults - very unlikely to overlap as a reserved word
        let ciphers = readCiphers(options.ciphers);
        if (ciphers == null || ciphers.length==0) {
          ciphers = readCiphers(constants.HTTPS_DEFAULT_CIPHERS);
        }
        this.httpsOptions.ciphers = ciphers;
      }
      bootstrapLogger.debug('Using tls ciphers:',this.httpsOptions.ciphers);

      //see https://nodejs.org/docs/latest-v16.x/api/tls.html#tlscreatesecurecontextoptions
      if (options.curves && Array.isArray(options.curves)) {
        this.httpsOptions.ecdhCurve = options.curves.join(':');
        bootstrapLogger.debug('Using ECDH curves:', this.httpsOptions.ecdhCurve);
      }
      ...
      if (util.isServerHttps(zoweConfig) || !(util.isClientAttls(zoweConfig))) {
        readTlsOptionsFromConfig(nodeConfig, this.httpsOptions, zoweConfig.zowe?.certificate?.keystore?.password, zoweConfig.zowe?.certificate?.keystore?.alias);
      }
