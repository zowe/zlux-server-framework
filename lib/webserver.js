...
function readTlsOptionsFromConfig(config, httpsOptions) {
    if (config.https.pfx) {
        ...
        if (config.https.certificates) {
            httpsOptions.cert = util.readFilesToArray(config.https.certificates);
...
class WebServer {
    constructor() {
        this.validateAndPreprocessConfig = BBPromise.coroutine(function* validateAndPreprocessConfig(config) {
            let canRun = false;
        ...
        this.startListening = BBPromise.coroutine(function* (app) {
                    ...
                    while (!listening) {
                        try {
                            ...
                            if (e.message == 'mac verify failure') {
                                const r = reader();
                                try {
                                    this.httpsOptions.passphrase = yield reader.readPassword('HTTPS key or PFX decryption failure. Please enter passphrase: ');
                                }
                                finally {
                                    r.close();
                                }
