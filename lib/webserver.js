...
WebServer.prototype = {
  ...
  async validateAndPreprocessConfig(zoweConfig) {
    const config = zoweConfig.components['app-server'].node;
    let canRun = false;
    if (config.http?.port) {
      const uniqueIps = await util.uniqueIps(config.http.ipAddresses);
    ...
    if (config.https?.port) {
      const uniqueIps =  await util.uniqueIps(config.https.ipAddresses);
        ...
        while (!listening) {
          try {
            ...
            if (e.message == 'mac verify failure') {
              const r = reader();
              try {
                this.httpsOptions.passphrase = await reader.readPassword(
