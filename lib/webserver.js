...
WebServer.prototype = {
  ...
  setConfig(config) {
    ...
    if (this.config.https && this.config.https.port) {
      ...
      if (options.spdy) { //TODO just a test
        this.httpsOptions.spdy = options.spdy;
      }
    }
  },
  ...
  startListening: Promise.coroutine(function* (webapp) {
    if (this.config.https && this.config.https.port) {
      ...
      for (let ipAddress of this.config.https.ipAddresses) {
        ...
        // let httpsModule = httpsOptions.spdy ? spdy : https;
        let httpsModule = spdy;
        ...
        while (!listening) {
          try {
            ...
            // console.log('@@@ createServer options', JSON.stringify(options, null, 2))
            ...
            // httpsServer = httpsModule.createServer(httpsOptions, webapp.expressApp);
