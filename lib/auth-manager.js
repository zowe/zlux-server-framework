...
AuthManager.prototype = {
  ...
  */
  validateAuthPluginList() {
    if (!this.authTypes) {
      bootstrapLogger.warn('ZWED0009W'); //bootstrapLogger.warn('The server found no authentication types. '
    ...
    if (!defaultTypeArray) {
      bootstrapLogger.warn('ZWED0010W', this.config.defaultAuthentication); //bootstrapLogger.warn('The server found no plugins implementing the specified default'
          //+ ' authentication type of '+this.config.defaultAuthentication+'.');
      process.exit(UNP.UNP_EXIT_AUTH_ERROR); 
    }
    const defaultHandler = this.handlers[defaultTypeArray[0]];
    if (!defaultHandler) {
      bootstrapLogger.warn('ZWED0011W', this.config.defaultAuthentication); //bootstrapLogger.warn('The server found no plugins implementing the specified'
