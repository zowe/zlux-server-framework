...
AuthManager.prototype = {
  ...
  getRequestedAuthCategories(startupPlugins, componentConfig) {
    const requestedCategories = [componentConfig.node.dataserviceAuthentication.defaultAuthentication];
  ...
  loadAuthenticators: Promise.coroutine(function*(config, tlsOptions, startupPlugins) {
    ...
    while ((plugin = this.pendingPlugins.pop()) !== undefined) {
      try {
        ...
        const capabilities = typeof authenticationHandler.getCapabilities === 'function' ? authenticationHandler.getCapabilities() : undefined;
        const haCompatible = capabilities && capabilities.haCompatible;
        if (isHaMode && !haCompatible) {
