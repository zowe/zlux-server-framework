...
var log = zluxUtil.loggers.storeLogger;
  ...
  if (this.isLocalStorage()) {
    setInterval(function() {
      ...
      if (this.sessions.size > 10000) {
        log.debug("ZWED0126I", this.sessions.size);//console.log('SessionStore.sessions.size=' + this.sessions.size);
      }
      ...
      this.sessions.forEach((session,key,map)=>
      {
        ...
        if (checkTimestamp - session.lastTouch > defaultMaxAge2) {
          map.delete(key);
          log.debug("ZWED0127I", session, key, map.size); //console.log('Timeout cleaner dead session=' + session + ' key=' + key + ' map.size=' + map.size);
        }
      });
    }.bind(this), 10000);
  }
...
SessionStore.prototype.clusterRemoteCall = function(methodName, args, callback) {
    process.clusterManager.callClusterMethodRemote('./sessionStore', "sessionStore", methodName, args,
        callback,
        function(e) {
            log.warn("ZWED0152W", methodName, e); //console.log("Error at call sessionStore." + methodName + ': ' + e);
        }
    );
}
...
SessionStore.prototype.set = function(sid, session, callback) {
  sync.updateSession(sid, session);
...
sync.syncEmitter.on('session', data => {
  console.log(`sync session ${data.sid} ${JSON.stringify(data.session)}`);
  sessionStore.set(data.sid, data.session, ()=> {});
});
sync.emitter.on('connected', function(ws) {
  console.log(`new client connected. send all sessions to the client `);
  sessionStore.all((err, sessions) => {
    console.log(`all ${JSON.stringify(err)} ${JSON.stringify(sessions)}}`);
    const data = [];
    Object.keys(sessions).forEach(sid => {
      const session = sessions[sid];
      data.push({sid, session});
    });
    console.log(`send all sessions as array ${JSON.stringify(data)}`);
    sync.updateSessionsForNewClient(ws, data)
  });
});
