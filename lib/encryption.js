...
const PBKDF2_DEFAULT_ROUNDS = 100000;

function getKeyFromPassword(password,salt,length,callback,rounds) {
  var iterations = (typeof rounds === 'number' && rounds > 0) ? rounds : PBKDF2_DEFAULT_ROUNDS;
  crypto.pbkdf2(password,salt,iterations,length,'sha256',(error, derivedKey) => {
    if (error) {
      throw error;
    }
    else {
