...
function makeCatalogForPlugin(plugin, productCode, nodeContext, hostsContext) {
  return new Promise((resolve) => {
    ...
    if(plugin.identifier === 'org.zowe.zss.swagger') {
      openApi.basePath = '/';
      openApi.host = getProxiedHost(hostsContext);
      openApi.schemes = getProxiedSchemesFromContext(hostsContext);
    }
...
var getSwaggerDocs = Promise.coroutine(function* (plugin, productCode, nodeContext, hostsContext) {
  var allServiceDocs = [];
  if (plugin.dataServices || plugin.swagger) {
    let serviceList = plugin.dataServices || plugin.swagger;
...
function overwriteSwaggerFieldsForServer (swaggerJson, urlBase, nodeContext, hostsContext) {
  ...
  if(urlBase.endsWith('org.zowe.zss.swagger')) {
