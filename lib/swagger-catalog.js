...
function getHost(serverConfig) {
  let hostname = serverConfig.zowe.externalDomains[0];
  return serverConfig.components['app-server'].node.https
    ? `${hostname}:${serverConfig.components['app-server'].node.https.port}`
    : `${hostname}:${serverConfig.components['app-server'].node.http.port}`;
}
