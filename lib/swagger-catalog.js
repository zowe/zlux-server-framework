...
function overwriteSwaggerFieldsForAgent(swaggerJson, plugin, hostsContext) {
  if(plugin.swagger && plugin.swagger.host == 'agent') {
    ...
    swaggerJson.host = getAgentHost(hostsContext);
    swaggerJson.schemes = getAgentSchemesFromContext(hostsContext);
  }
  return swaggerJson;
}
...
function getAgentHost(hostsContext) {
  return `${hostsContext.agentHost}:${hostsContext.agentPort}`
}

function getAgentSchemesFromContext (hostsContext) {
  let schemes = [hostsContext.isAgentHttps?'https':'http'];
  return schemes;
}
