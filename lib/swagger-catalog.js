  ...
  return new Promise((resolve) => {
    ...
    getSwaggerDocs(plugin, productCode, nodeContext).then((swaggerDocs) => {
      swaggerDocs.forEach((service)=> {
        ...
        let version = service.serviceDoc.info.version;
...
async function getSwaggerDocs(plugin, productCode, nodeContext) {
  ...
  if (plugin.dataServices){
    for (let i = 0; i < plugin.dataServices.length; i++) {
      ...
      let fileContent = await readSingleSwaggerFile(path.join(plugin.location, "doc/swagger"), service.name)
          .catch((err) => {
            if (err.code === 'ENOENT') {
...
function readSingleSwaggerFile (dirName, serviceName) {
  // read one swagger file and validate the json that is returned
  return new Promise ((resolve, reject) => {
    const jsonPath = path.join(dirName,serviceName+'.json');
    const yamlPath = path.join(dirName,serviceName+'.yaml');
    installLog.debug(`Reading swagger at paths=`,jsonPath,yamlPath);
    fs.readFile(jsonPath,{encoding:'utf-8'},(err,fileContent)=> {
      if (err) {
        installLog.debug(`JSON not found or invalid, attempting YAML swagger at ${yamlPath}`);
        fs.readFile(yamlPath,{encoding:'utf-8'},(err, fileContent)=> {
          if (err) {
            return reject(err);
          }
