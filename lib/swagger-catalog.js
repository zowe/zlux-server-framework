...
function makeCatalogForPlugin(plugin, productCode, nodeContext, hostsContext) {
  return new Promise((resolve) => {
    const openApi = {
      ...
      host: getHost(nodeContext, hostsContext),
      schemes: getSchemesFromContext(nodeContext),
      paths: {}
    };

    getSwaggerDocs(plugin, productCode, nodeContext, hostsContext).then((swaggerDocs) => {
      let skipRouteServicePrefix = false;

      overwriteSwaggerFieldsForAgent(openApi, plugin, hostsContext);
      if(plugin.swagger && plugin.swagger.basePath > '') {
        skipRouteServicePrefix = true;
      }
...
var getSwaggerDocs = Promise.coroutine(function* (plugin, productCode, nodeContext, hostsContext) {
  var allServiceDocs = [];
  if (plugin.dataServices || (plugin.swagger && plugin.swagger.docs)) {
    let serviceList = plugin.dataServices || plugin.swagger.docs;
    ...
    for (let i = 0; i < serviceList.length; i++) {
      ...
      try {
        ...
        if (err.code === 'ENOENT') {
          installLog.warn(`ZWED0047W`, plugin.identifier, service.name); //installLog.warn(`Swagger file for service (${plugin.identifier}:${service.name}) not found`);
        } else {
      ...
      if (fileContent) {
        fileContent = overwriteSwaggerFieldsForServer(fileContent,
                                                      ...
                                                      nodeContext, hostsContext);
        fileContent = overwriteSwaggerFieldsForAgent(fileContent, plugin, hostsContext);
...
function readSingleSwaggerFile (dirName, serviceName, serviceVersion) {
  ...
  return new Promise ((resolve, reject) => {
    ...
    fs.readdir(dirName,(err, files)=> {
      if (err) {
        installLog.warn(`ZWED0050W`, dirName); //installLog.warn(`Could not read swagger doc folder ${dirName}`);
        return reject(err);
      } else {
...
function overwriteSwaggerFieldsForAgent(swaggerJson, plugin, hostsContext) {
  if(plugin.swagger && plugin.swagger.host == 'agent') {
    swaggerJson.basePath = plugin.swagger.basePath || swaggerJson.basePath;
    swaggerJson.host = getProxiedHost(hostsContext);
    swaggerJson.schemes = getProxiedSchemesFromContext(hostsContext);
  }
  return swaggerJson;
}

function overwriteSwaggerFieldsForServer (swaggerJson, urlBase, nodeContext, hostsContext) {
  ...
  swaggerJson.host = getHost(nodeContext, hostsContext);
  return swaggerJson;
}
...
function getHost(nodeContext, hostsContext) {
  let hostname = os.hostname();

  if(hostsContext && hostsContext.hostname) {
    hostname = hostsContext.hostname
  } 
...
function getProxiedHost(hostsContext) {
  return `${hostsContext.proxiedHost}:${hostsContext.proxiedPort}`
}

function getProxiedSchemesFromContext (hostsContext) {
  let schemes = [hostsContext.isProxiedHttps?'https':'http'];
  return schemes;
}
