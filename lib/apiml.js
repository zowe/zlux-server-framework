...
ApimlConnector.prototype = {
  ...
  _makeMainInstanceProperties(overrides) {
    ...
    protocolObject.httpEnabled = this.httpPort ? true : false;
    protocolObject.httpsEnabled = this.httpsPort ? true : false;

    let proto = protocolObject.httpsEnabled ? 'https' : 'http';
    let port = protocolObject.httpsEnabled ? protocolObject.httpsPort : protocolObject.httpPort;
    
    // This is a workaround for routing issues in the API ML
    // If the HTTP port is set to 0 then the API ML can't load zlux
    let mlHttpPort;
    if(protocolObject.httpEnabled) {
      mlHttpPort = Number(this.httpPort);
    } else {
      mlHttpPort = Number(this.httpsPort);
    }

    log.info("Protocol:", proto, "Port", port);
    log.info("Protocol Object:", JSON.stringify(protocolObject));
    ...
    Object.assign(instance,  {
     ...
     log.info("API ML registration settings:", JSON.stringify(instance));
  ...
  registerMainServerInstance() {
    ...
    log.info(`zluxProxyServerInstanceConfig ${JSON.stringify(zluxProxyServerInstanceConfig)}`)
