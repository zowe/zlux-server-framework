...
ApimlConnector.prototype = {
  ...
  waitUntilZluxClusterIsReady(count) {
    return this.getZluxInstances().then(
      zluxInstances => {
        if (zluxInstances.length < count) {
          return this.waitUntilZluxClusterIsReady(count);
        }
  ...
  takeInstanceIntoService(instanceId) {
    log.info(`takeInstanceIntoService ${instanceId}`);
  ...
  _overrideStatusForInstance(instanceId, status) {
    return new Promise((resolve, reject) => {
      this.zluxServerEurekaClient.eurekaRequest({
        ...
        if (err) {
          log.debug(`_overrideStatus error ${JSON.stringify(err)}`);
          reject(err);
        } else {
          resolve();
        }
  ...
  _deleteOverriddenStatusForInstance(instanceId) {
    return new Promise((resolve, reject) => {
      this.zluxServerEurekaClient.eurekaRequest({
        method: 'DELETE',
        uri: `${this.zluxServerEurekaClient.config.instance.app}/${instanceId}/status`
      }, (err) => {
        if (err) {
