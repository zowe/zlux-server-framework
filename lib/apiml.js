...
ApimlConnector.prototype = {
  ...
  async setBestIpFromConfig(nodeConfig) {
    const nodeIps = await zluxUtil.uniqueIps(nodeConfig.https && nodeConfig.https.ipAddresses ? nodeConfig.https.ipAddresses : nodeConfig.http.ipAddresses);
    const eurekaIp = await zluxUtil.uniqueIps([nodeConfig.mediationLayer.server.hostname]);
