...
ApimlConnector.prototype = {
  ...
  registerMainServerInstance(instanceConfigOverrides) {
    ...
    const zluxProxyServerInstanceConfig = {
      instance: this._makeMainInstanceProperties(instanceConfigOverrides),
  ...
  waitUntilRaftClusterIsReady(clusterSize) {
    return this.getZluxInstances().then(
      zluxInstances => {
        if (zluxInstances.length < clusterSize) {
          return this.waitUntilRaftClusterIsReady(clusterSize);
        }
...
function makeApiml(userConfig) {
  const wsConfig = userConfig.node;
  const apimlConfig = userConfig.node.mediationLayer;
  const { httpOptions, httpsOptions } = webserver.makeWebServerHttpAndHttpsOptions(wsConfig);
  let apimlTlsOptions;
  ...
  const apiml = new ApimlConnector({
...
module.exports.makeApiml = makeApiml;
