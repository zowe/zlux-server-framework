...
module.exports.resolveRelativePathAgainstCWD = function resolveRelativePathAgainstCWD(path) {
...
const loggers = {
  ...
  network: global.COM_RS_COMMON_LOGGER.makeComponentLogger("_unp.network"),
};

module.exports.loggers = loggers;

module.exports.resolveRelativePaths = function resolveRelativePaths(root, resolver, key) {
  for (const key of Object.keys(root)) {
    const value = root[key];
    const valueType = typeof value;
    if (valueType == 'object') {
      resolveRelativePaths(value, resolver);
    } else if ((valueType == 'string') && value.startsWith('../')) {
      const old = root[key];
      root[key] = resolver(value, key);
      loggers.utilLogger.info(`Resolved path: ${old} -> ${root[key]}`);
    }
  }
};
