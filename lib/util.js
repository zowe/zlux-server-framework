...
module.exports.makeRemoteUrl = function(destination,req) {
  ...
  if(destination.includes('ZOWE_REFERRER_HOST_PORT') && referer) {
    let referrerHostPort = referer.split('/')[2];
    //dont mix this with other vars, so return
    return destination.replace('${ZOWE_REFERRER_HOST_PORT}', referrerHostPort);
  }
...
module.exports.getCookieName = (port) => {
  const zoweInstanceId = process.env['ZOWE_INSTANCE'];
  const id = isHaMode() ? zoweInstanceId : String(port);
  return 'connect.sid.' + id;
}
