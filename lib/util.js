...
module.exports.DataserviceStorage = function(pluginId) {
  return new DataserviceStorage(pluginId);
}

function DataserviceStorage(pluginId) {
  this.storageDict = new Object;
  if (pluginId) {
    this.pluginId = pluginId;
  }

  this.getStorage = function (pluginId) {
  ...
  this.setStorageValue = function (id, value, pluginId) {
    if (pluginId === undefined) {
      const mergedObjects = mergeUtils.deepAssign(this.storageDict[this.pluginId], {
        [id]: value
      });
      this.storageDict[this.pluginId] = mergedObjects;
    } else {
      const mergedObjects = mergeUtils.deepAssign(this.storageDict[pluginId], {
        [id]: value
      });
      this.storageDict[pluginId] = mergedObjects;
    }
  }

  this.setStorage = function (dict, pluginId) {
    if (pluginId === undefined) {
      this.storageDict[this.pluginId] = dict;
    } else {
      this.storageDict[pluginId] = dict;
    }
  }

  this.getStorageValue = function (id, pluginId) {
    if (pluginId === undefined) {
      return this.storageDict[this.pluginId][id];
    } else {
      return this.storageDict[pluginId][id];
    }
  }

  this.deleteStorageValue = function (id, pluginId) {
    if (pluginId === undefined) {
      delete this.storageDict[this.pluginId][id];
    } else {
      delete this.storageDict[pluginId][id];
    }
  }
}
