    ...
    try {
      ...
      if(extension == 'p12' || extension == 'pfx'){
        ...
        try{
          ...
          loggers.bootstrapLogger.warn("ZWED0173W", e.message);
        ...
        for(let i = 0; i < certData.length; i++){
          let currentCert = certData[forge.pki.oids.certBag][i];
          contentArray.push(Buffer.from(forge.pki.certificateToPem(currentCert.cert), 'utf8'));
        }
      } else {
