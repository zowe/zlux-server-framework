  ...
  fileList.forEach(function(filePath) {
    try {
      ...
      if(extension == 'p12'){
        ...
        console.log('keystore password: ', process.env.KEYSTORE_PASSWORD);
        let p12 = forge.pkcs12.pkcs12FromAsn1(p12Asn1, "");
        let keyData = p12.getBags({bagType: forge.pki.oids.certBag});
        let certBag = keyData[forge.pki.oids.certBag][0];
        contentArray.push(Buffer.from(forge.pki.certificateToPem(certBag.cert), 'utf8'));
        //logger.bootstrapLogger.debug(forge.asn1.prettyPrint(p12Asn1))
      } else {
