...
module.exports.getRemoteIframeTemplate = function() {
  let defaultRemoteAppTemplate = fs.readFileSync(path.join(__dirname,'default-remote-app-template.html'),"utf-8");
  return (templatePath) => {
    let remoteAppTemplate = defaultRemoteAppTemplate;
    if(templatePath) {
      remoteAppTemplate = fs.readFileSync(templatePath, "utf-8");
    }
    return (remoteUrl) => {
      return remoteAppTemplate.replace('${remoteUrl}', remoteUrl);
    }
  }
}();
...
module.exports.makeRemoteUrl = function(destination,req) {
  return destination
          .replace('${ZOWE_EXTERNAL_HOST}', req.host)
