...
module.exports.getHostAndPortFromUrl = function getHostAndPortFromUrl(url) {
  ...
  if (lastColonIndex == -1) {
    ...
    const portStr = hostAndPort.substring(lastColonIndex+1);
    const portNum = parseInt(portStr, 10);
    return {
      //strip [] around ipv6 if exists
      host: hostAndPort.substring(0, lastColonIndex).replace('[', '').replace(']', ''),
      port: Number.isNaN(portNum) ? portStr : portNum
    }
  }
}
...
module.exports.resolveRelativePaths = function resolveRelativePaths(root, resolver, relativeTo, _seen) {
  ...
  for (const key of Object.keys(root)) {
    ...
    if (valueType == 'object' && value) {
      resolveRelativePaths(value, resolver, relativeTo, _seen);
...
module.exports.clone = function(obj) {
  try {
    ...
    // Fallback for objects that structuredClone cannot handle (e.g. functions)
...
module.exports.readOnlyProxy = function readOnlyProxy(obj) {
  return new Proxy(obj, {
    ...
    set: function(target, property, value) {
      throw new TypeError(`Cannot set property '${String(property)}' on read-only proxy`);
    },
    deleteProperty: function(target, property) {
      throw new TypeError(`Cannot delete property '${String(property)}' on read-only proxy`);
    }
  });  
};
...
module.exports.getRemoteIframeTemplate = function(remoteUrl) {
  // Sanitize URL to prevent XSS injection in HTML context
  const sanitized = String(remoteUrl)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#x27;');
  return defaultRemoteAppTemplate.replace('${remoteUrl}', sanitized);
}
