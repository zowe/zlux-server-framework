...
function makeSimpleProxy(host, port, options, pluginID, serviceName) {
    ...
    const { urlPrefix, isHttps, addProxyAuthorizations, allowInvalidTLSProxy } = options;
...
function makeWsProxy(host, port, urlPrefix, isHttps) {
    ...
    var handleProxyWSException = function (e, ws, proxyws) {
        ...
        try {
            if (ws) {
                ws.close(unpconst.WEBSOCKET_CLOSE_INTERNAL_ERROR, JSON.stringify({ error: 'Internal Server Error' }));
            }
            if (proxyws) {
                proxyws.close(unpconst.WEBSOCKET_CLOSE_INTERNAL_ERROR, JSON.stringify({ error: 'Internal Server Error' }));
            }
        }
        catch (closeEx) {
