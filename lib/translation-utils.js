...
function loadTranslations(pluginLocation) {
    ...
    const fileExt = '.json';
    const pattern = path.join(pluginLocation, relativePath, `${filePrefix}*${fileExt}`);
    const files = glob.sync(pattern, {});
    for (const file of files) {
        const basename = path.basename(file);
        const languageCountry = basename.substring(filePrefix.length, basename.length - fileExt.length);
        let content;
        try {
...
function translate(pluginDef, translationMaps, acceptLanguage) {
    ...
    const langCountry = acceptLanguageParser.pick(availableTranslations, acceptLanguage);
    if (!langCountry) {
        return pluginDef;
    }
...
exports.getAcceptLanguageFromCookies = getAcceptLanguageFromCookies;
...
function translateObject(object, translationMap) {
    for (const key in object) {
        if (key.endsWith('Key') && typeof object[key] === 'string') {
            ...
            if (typeof object[keyToTranslate] === 'string' &&
...
function unitTest() {
    ...
    const translatedPluginDefinition = translate(pluginDefinition, translationMaps, acceptLanguage);
