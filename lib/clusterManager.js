...
if (cluster.isMaster) {
  ...
  ClusterManager.prototype.getStorage = function(resultHandler) {
    if (this.storage) {
      return resultHandler(this.storage);
    } else {
      return resultHandler(zluxUtil.DataserviceStorage());
    }
  }

  ClusterManager.prototype.setStorageValue = function(id, value, resultHandler) {
    if (this.storage) {
      this.storage.setStorageValue(id, value);
      return resultHandler(true);
    } else {
      clusterLogger.warn("ZWED0160W - Master cluster not found to save storage for id: " + id + ", value: " + value);
      return resultHandler(false);
    }
  }

  ClusterManager.prototype.setStorage = function(dict, resultHandler) {
    if (this.storage) {
      this.storage.setStorage(dict);
      return resultHandler(true);
    } else {
      clusterLogger.warn("ZWED0161W - Master cluster not found to save storage");
      return resultHandler(false);
    }
  }

  ClusterManager.prototype.addStorage = function(dict, resultHandler) {
    if (this.storage) {
      const mergedObjects = recursiveMerge(this.storage.getStorage(), dict);
      this.storage.setStorage(mergedObjects);
      return resultHandler(true);
    } else {
      clusterLogger.warn("ZWED0162W - Master cluster not found to add storage");
      return resultHandler(false);
    }
  }

  ClusterManager.prototype.getStorageValue = function(id, resultHandler) {
    if (this.storage) {
      return resultHandler(this.storage.getObject(id));
    } else {
      return resultHandler(null);
    }
  }

  ClusterManager.prototype.deleteStorageValue = function(id) {
    if (this.storage) {
      this.storage.deleteObject(id);
    } else {
      clusterLogger.warn("ZWED0163W - Master cluster was not found to delete storage for id: ", id);
    }
  }
  ...
  ClusterManager.prototype.getStorage = function() {
    return this.callClusterMethodRemote(null, "clusterManager", "getStorage", [],
      function(resultHandler) { 
        return resultHandler[0]; // Return the first arg (storage object) of getStorage
      },
      function(e) {
        clusterLogger.warn("ZWED0164W - Unable to retrieve storage object from cluster", e);
      }, 5000 /* We need to give ample timeout time in case we attempt to retrieve storage
      before the cluster is done making it. */
    ).then((storage) => {
      return storage;
    }).catch((e) => {
      clusterLogger.warn("ZWED0165W - Unable to retrieve storage data from cluster promise", e);
      return e;
    })
  }

  /* Though set here can be called like sync, we provide a callback in case we need
  to wait for a set response */
  ClusterManager.prototype.setStorageValue = function(id, value) {
    return this.callClusterMethodRemote(null, "clusterManager", "setStorageValue", [id, value],
      function(resultHandler) {
  ...
  /* Though set here can be called like sync, we provide a callback in case we need
  to wait for a set response */
  ClusterManager.prototype.setStorage = function(dict) {
    return this.callClusterMethodRemote(null, "clusterManager", "setStorage", [dict],
      function(resultHandler) {
        return resultHandler[0];
      },
      function(e) {
  ...
  /* Though add here can be called like sync, we provide a callback in case we need
  to wait for an add response */
  ClusterManager.prototype.addStorage = function(dict) {
    return this.callClusterMethodRemote(null, "clusterManager", "addStorage", [dict],
      function(resultHandler) {
        return resultHandler[0];
      },
      function(e) {
  ...
  ClusterManager.prototype.getStorageValue = function(id) {
    return this.callClusterMethodRemote(null, "clusterManager", "getStorageValue", [id],
    function(resultHandler) { 
      return resultHandler[0]; // Return the first arg (storage object) of getStorage
    },
    function(e) {
  ...
  ClusterManager.prototype.deleteStorageValue = function(id) {
    this.callClusterMethod(null, "clusterManager", "deleteStorageValue", [id],
      function() {
      },
      function(e) {
        clusterLogger.warn("ZWED0169W - Error deleting storage with id: " + [id] + " " + e);
      }
    );
  }
}
...
/* Performs a recursive merge on objects, which is different from spread/Object.assign operations 
that do not merge n > 2 levels down. Does not modify original objects. */
function recursiveMerge(...objects) {
  const isObject = object => object && typeof object === 'object';
  
  return objects.reduce((final, merge) => {
    Object.keys(merge).forEach(key => {
      const finalValue = final[key];
      const mergeValue = merge[key];
      
      // If the values are arrays, combine them
      if (Array.isArray(finalValue) && Array.isArray(mergeValue)) {
        final[key] = finalValue.concat(...mergeValue);
      }
      // If we have more objects left, go deeper
      else if (isObject(finalValue) && isObject(mergeValue)) {
        final[key] = recursiveMerge(finalValue, mergeValue);
      }
      // Otherwise, merge
      else {
        final[key] = mergeValue;
      }
    });
