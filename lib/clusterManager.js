...
const makeApiml = require('./apiml').makeApiml;
...
if (cluster.isMaster) {
  ...
  ClusterManager.prototype.initializing = function(completeHandler) {
    ...
    inits.push(this.initSecret.bind(this));
    if (process.env.ZOWE_SESSION_SECRET) {
  ...
  ClusterManager.prototype.setStorageByKeyLocal = function(pluginId, key, value) {
    this.storage.set(key, value, pluginId);
  }
  
  ClusterManager.prototype.setStorageByKey = function(pluginId, key, value, resultHandler) {
    this.setStorageByKeyLocal(pluginId, key, value);
    sync.setStorageForPlugin(pluginId, key, value);
    return resultHandler(true);
  }

  ClusterManager.prototype.setStorageAllLocal = function(pluginId, dict) {
    this.storage.setAll(dict, pluginId);
  }
  
  ClusterManager.prototype.setStorageAll = function(pluginId, dict, resultHandler) {
    this.setStorageAllLocal(pluginId, dict);
    sync.setAllStorageForPlugin(pluginId, dict);
    return resultHandler(true);
  }
  ...
  ClusterManager.prototype.deleteStorageByKeyLocal = function(pluginId, key) {
    this.storage.delete(key, pluginId);
  }
  
  ClusterManager.prototype.deleteStorageByKey = function(pluginId, key, resultHandler) {
    this.deleteStorageByKeyLocal(pluginId, key);
    sync.deleteStorageForPlugin(pluginId, key);
    return resultHandler(true);
  }
...
ClusterManager.prototype.start = function(appConfig, configJSON, startUpConfig, configLocation) {
  ...
  if (cluster.isMaster) {
    ...
    this.initializing(function() {
      this.startWorkers();
      this.startApimlAndRaft(configJSON);
    }.bind(this));
  } else {
...
ClusterManager.prototype.startApimlAndRaft = Promise.coroutine(function* (configJSON) {
  const mediationLayerEnabled = configJSON.node.mediationLayer && configJSON.node.mediationLayer.enabled;
  if (mediationLayerEnabled) {
    const apiml = makeApiml(configJSON);
    yield apiml.setBestIpFromConfig(configJSON.node);
    const overrides = raft.isEnabled ? {overriddenStatus: 'OUT_OF_SERVICE'} : undefined;
    yield apiml.registerMainServerInstance(overrides);
    if (raft.isEnabled) {
      yield raft.start(apiml);
    }
  }
}),
