...
if (cluster.isMaster) {
  ...
  ClusterManager.prototype.getStorageByKey = function(pluginId, key, resultHandler) {
    if (this.storage) {
      return resultHandler(this.storage.getStorageByKey(key, pluginId));
    } else {
  ...
  ClusterManager.prototype.getStorageAll = function(id, resultHandler) {
    if (this.storage) {
      return resultHandler(this.storage.getStorageAll(id));
    } else {
      return resultHandler(zluxUtil.DataserviceStorage(id).getStorageAll());
    }
  }
  ...
  ClusterManager.prototype.getStorageCluster = function(resultHandler) {
    if (this.storage) {
      ...
      return resultHandler(zluxUtil.DataserviceStorage().storageDict);
    }
  }

  ClusterManager.prototype.setStorageByKey = function(pluginId, key, value, resultHandler) {
    if (this.storage) {
      this.storage.setStorageByKey(key, value, pluginId);
      return resultHandler(true);
    } else {
      clusterLogger.warn("ZWED0160W - Master cluster not found to save storage for key: " + key + ", value: " + value);
  ...
  ClusterManager.prototype.setStorageAll = function(pluginId, dict, resultHandler) {
    if (this.storage) {
      const allStorage = this.storage.getStorageAll(pluginId);
      allStorage[pluginId] = dict;
      this.storage.setStorageAll(allStorage);
      return resultHandler(true);
    } else {
  ...
  ClusterManager.prototype.mergeStorage = function(pluginId, dict, resultHandler) {
    if (this.storage) {
      ...
      if (allStorage == null || allStorage == dict) {
        this.storage.setStorageAll(dict, pluginId);
      } else {
        const mergedObjects = mergeUtils.deepAssign(allStorage, dict);
        this.storage.setStorageAll(mergedObjects, pluginId);
      }
      return resultHandler(true);
    } else {
  ...
  ClusterManager.prototype.deleteStorageByKey = function(pluginId, key, resultHandler) {
    if (this.storage) {
      this.storage.deleteStorageByKey(key, pluginId);
      return resultHandler(true);
    } else {
      clusterLogger.warn("ZWED0163W - Master cluster was not found to delete storage for id: ", key);
      return resultHandler(false);
    }
  }
  ...
  ClusterManager.prototype.getStorageAll = function(id) {
    return this.callClusterMethodRemote(null, "clusterManager", "getStorageAll", [id],
      ...
      function(e) {
        ...
        return e;
      }, 5000 /* We need to give ample timeout time since this method is used to verify 
      the completion of cluster storage initialization in webapp.js */
    )
  }
