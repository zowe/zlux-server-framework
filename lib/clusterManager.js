...
let minWork = process.env.ZLUX_MIN_WORKERS;
let maxWork = process.env.ZLUX_MAX_WORKERS;

if (minWork) {
  if (minWork < 1) {
    clusterLogger.info("Environmental variable ZLUX_MIN_WORKERS is set to below 1 and will not be used. Minimum workers will be set to 1.")
    minWork = 1;
  }
  if (maxWork) {
    if (parseInt(maxWork) > cpuCount*2) {
      clusterLogger.info("Environmental variable ZLUX_MAX_WORKERS is set to more than twice the cpuCount and will not be used. Maximum workers will be set to twice the cpuCount.")
      maxWork = cpuCount*2
    }
    if (parseInt(minWork) > parseInt(maxWork)) {
      clusterLogger.info("Environmental variable ZLUX_MIN_WORKERS is set to more than the maximum workers and will not be used. Minimum workers will be set to the maximum workers.")
      minWork = maxWork;
    }
  } else if (parseInt(minWork) > cpuCount*2) {
    clusterLogger.info("Environmental variable ZLUX_MIN_WORKERS is set to more than twice the cpuCount and will not be used. Minimum workers will be set to twice the cpuCount.")
    minWork = cpuCount*2
  }
}

const minWorkers = minWork || 1;
const maxWorkers = maxWork || cpuCount;
...
if (cluster.isMaster) {
  ...
  ClusterManager.prototype.reloadAllWorkers = function() {
    ...
    clusterLogger.info(`keys=`,workerKeys);
