...
const defaultMaxAge = Number(process.env.sessionMaxAge);
const defaultMaxAge2 = Number(process.env.sessionMaxAge2 || 1000*60*60*24*7);//week
const storedSessionsLimit = Number(process.env.storedSessionsLimit || 500000);
...
class SessionStore extends events.EventEmitter {
  ...
  isLocalStorage() {
    return !(process as any).clusterManager || (process as any).clusterManager.isPrimary;
  }
  ...
  clusterRemoteCall(methodName, args, callback) {
