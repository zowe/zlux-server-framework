...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {
    ...
    const cookieIdentifier = this.userConfig.cookieIdentifier;
    const webAppOptions = {
    ...
    if (usingApiml) {
      ...
      if (firstWorker) {
        ...
        webAppOptions.gatewayRedirect = `${this.userConfig.node.mediationLayer.server.isHttps === false ? 'http' : 'https' }://`
          +`${this.userConfig.node.mediationLayer.server.gatewayHost}:${this.userConfig.node.mediationLayer.server.gatewayPort}`
          +`/${util.getPrefixForService('zlux', 'ui', 'v1')}`;
      }
    ...
    const proxiedOptions = util.getAgentRequestOptions(this.userConfig, webAppOptions.tlsOptions, false);
