...
class Server {
    constructor(appConfig, userConfig, startUpConfig, options) {
        //Might need a decerator/* because of generator 
        this.start = BBPromise.coroutine(function* () {
    ...
    pluginLoaded(pluginDef) {
        bootstrapLogger.warn('plugin with name ' + pluginDef.identifier + ' has loaded');
        const pluginContext = {
            pluginDef,
            server: {
                config: {
