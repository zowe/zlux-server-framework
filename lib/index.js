...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {    
    ...
    const healthRegistry = {
      // UP after all plugins finish loading (ZWED0031I)
      pluginsLoaded: { status: 'DOWN' }
    };
    ...
    if (apimlConfig.enabled) {
      // UP after registerMainServerInstance() succeeds
      healthRegistry.eurekaRegistration = { status: 'DOWN' };
      if (firstWorker) {
        ...
        healthRegistry.eurekaRegistration = { status: 'UP' };
      ...
      if (this.componentConfig.agent?.mediationLayer?.enabled
        ...
        // UP after checkAgent() confirms the agent is reachable via discovery
        healthRegistry.agentConnection = { status: 'DOWN' };
        ...
        if (firstWorker) {
          yield this.apiml.checkAgent(this.componentConfig.agent.handshakeTimeout,
                                      this.componentConfig.agent.mediationLayer.serviceName);
          healthRegistry.agentConnection = { status: 'UP' };
        }
      }
  ...
  pluginLoadingFinished(adr, percent, loaded, total) {
    ...
    if (this.healthRegistry) {
      this.healthRegistry.pluginsLoaded = { status: 'UP' };
    }
