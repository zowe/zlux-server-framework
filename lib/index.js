...
Server.prototype = {
  ...
  configureApimlStorage(apimlConfig) {
    ...
    if (typeof cachingService !== 'object') {
      return;
    }
    const {cert: clientCertFile, key: clientKeyFile} = cachingService;
    ...
    try {
      ...
      bootstrapLogger.warn(`ZWED0176W`, e.String()) // Failed to load client cert/key pair for Caching Service (%s)
      return;
    }
    if (key && cert && gatewayHost && gatewayPort) {
      const settings = {
        host: gatewayHost,
        port: gatewayPort,
        tlsOptions: {
