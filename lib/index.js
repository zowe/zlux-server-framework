...
Server.prototype = {
  ...
  async start() {    
    ...
    if (!(await this.webServer.validateAndPreprocessConfig(this.zoweConfig))) {
    ...
    if (apimlConfig.enabled) {
      if (firstWorker) {
        ...
        await this.apiml.setBestIpFromConfig(this.componentConfig.node);
        await this.apiml.registerMainServerInstance();
      ...
      if (this.componentConfig.agent?.mediationLayer?.enabled
        ...
        if (firstWorker) {
          await this.apiml.checkAgent(this.componentConfig.agent.handshakeTimeout,
                                      this.componentConfig.agent.mediationLayer.serviceName);
        }
      }
      ...
      if (firstWorker &&
          ...
          */
          await checkProxiedHost(webAppOptions.proxiedHost,
    ...
    await this.webServer.startListening(this.webApp);
    this.webApp.init();
    ...
    await this.authManager.loadAuthenticators(this.zoweConfig, Object.assign({},this.tlsOptions), plugins);
    ...
    for (let i = 0; i < this.langManagers.length; i++) {
      await this.langManagers[i].startAll();
    }
  },
  ...
  async loadPlugins() {
    ...
    return this.pluginLoader.loadPlugins();
  },
