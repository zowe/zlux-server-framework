  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    if (this.userConfig.node.mediationLayer.enabled
      ...
      let zluxAppServerClusterSize = +process.env.ZLUX_APP_SERVER_CLUSTER_SIZE;
      if (!Number.isInteger(zluxAppServerClusterSize) || zluxAppServerClusterSize < 3) {
        zluxAppServerClusterSize = 3;
      }
      bootstrapLogger.info(`my instance is ${instanceId}, zlux-app-server cluster size ${zluxAppServerClusterSize}`);
      const zluxInstances = yield this.apiml.waitUntilZluxClusterIsReady(zluxAppServerClusterSize);
