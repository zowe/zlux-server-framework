...
function stopAllLangManagers(langManagers) {
  console.log(`Stopping managers`);
  for (let i = 0; i < langManagers.length; i++) {
    console.log(`Stopping manager=${i}`);
    langManagers[i].stopAll();
    console.log(`Stopped manager=${i}`);
  }
}
...
function Server(appConfig, userConfig, startUpConfig, options) {
  ...
  if (userConfig.languages.java) {
    langManagers.push(new javaManager.JavaManager(userConfig.languages.java, userConfig.instanceDir));
  }
  //these come BEFORE process manager, since process manager ends process on completion
  process.on('SIGTERM', function() {
    console.log(`Sigterm`);
    stopAllLangManagers(langManagers); });
