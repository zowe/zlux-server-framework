...
function Server(zoweConfig, configLocation) {
  ...
  this.authManager = new AuthManager({
    ...
    sessionTimeoutMs: this.componentConfig.node.session?.timeoutMS || this.componentConfig.node.session?.cookie?.timeoutMS
  });
...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {    
    ...
    if (apimlConfig && apimlConfig.server) {
    ...
    if (apimlConfig && apimlConfig.enabled) {
      ...
      // If SSH tunnel config is present, open the tunnel before checking the agent
      const sshConf = this.componentConfig.agent.ssh;
      if (sshConf && sshConf.host && sshConf.user && sshConf.password) {
        const tunnel = new SshTunnel({
          sshHost: sshConf.host,
          sshPort: sshConf.port || 22,
          sshUser: sshConf.user,
          sshPassword: sshConf.password,
          remoteHost: this.componentConfig.agent.host || '127.0.0.1',
          remotePort: webAppOptions.proxiedPort,
          localPort: webAppOptions.proxiedPort,
          localHost: '127.0.0.1'
        });
        bootstrapLogger.info('ZWED0900I - Opening SSH tunnel to ' + sshConf.host + ' for agent port ' + webAppOptions.proxiedPort);
        yield tunnel.start();
        this._sshTunnel = tunnel;
        this.processManager.addCleanupFunction(() => tunnel.stop());
      }
      if (firstWorker &&
