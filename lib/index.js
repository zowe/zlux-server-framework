  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    if (this.userConfig.node.mediationLayer.enabled
      ...
      console.log(`my instance is ${instanceId}`);
      const zluxInstances = yield this.apiml.waitUntilZluxClusterIsReady(2);
      console.log(`zlux cluster is ready, instances ${JSON.stringify(zluxInstances, null, 2)}`);
      const me = zluxInstances.findIndex(instance => instance.instanceId === instanceId);
      console.log(`my index is ${me}`);
      const peers = zluxInstances.map(instance => RaftPeer.make(instance));
      if (me !== -1) {
       raft.start(peers, me);
