...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {    
    ...
    const proxiedOptions = util.getAgentRequestOptions(this.zoweConfig, tlsOptions, false);
    ...
    yield this.authManager.loadAuthenticators(this.zoweConfig, Object.assign({},tlsOptions));
  ...
  loadPlugins: Promise.coroutine(function*() {
    ...
    const hosts = hostsContext(this.webApp.options);
    const homepage = this.componentConfig.node.mediationLayer.enabled
          ? util.getGatewayUrlForService('zlux', 'ui', 1)+'/'
    ...
    this.pluginLoader.on('pluginFound', util.asyncEventListener(event => {
      ...
      const percentLoaded = `${Math.round((pluginsLoaded/event.count)*100)}% (${pluginsLoaded}/${event.count})`;
      ...
      function handleIfComplete() {
        if (pluginCount === event.count) {
          if (!messageIssued) {
            this.pluginLoadingFinished(homepage, Math.round((pluginsLoaded/event.count)*100), pluginsLoaded, event.count);
            messageIssued = true;
          } else {
