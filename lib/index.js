  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    if (this.userConfig.node.mediationLayer.enabled
      ...
      let clusterSize = +process.env.ZLUX_SERVER_CLUSTER_SIZE;
      if (!Number.isInteger(clusterSize) || clusterSize < 3) {
        clusterSize = 3;
      }
      bootstrapLogger.info(`my instance is ${instanceId}, zlux cluster size ${clusterSize}`);
      const zluxInstances = yield this.apiml.waitUntilZluxClusterIsReady(clusterSize);
