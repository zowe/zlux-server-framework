  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    this.pluginLoader.on('pluginAdded', util.asyncEventListener(event => {
      let pluginLength = Object.keys(webAppOptions.pluginLoader.pluginMap).length;
      pluginCount++;
      return this.pluginLoaded(event.data).then(() => {
        pluginsLoaded++;
        installLogger.info('Installed plugin: ' + event.data.identifier + " (" + pluginsLoaded +"/" + pluginLength +")");
        if (pluginCount == Object.keys(webAppOptions.pluginLoader.pluginMap).length) {
          installLogger.info(`Server is ready at ` +
            `${webAppOptions.serverConfig.node.https.ipAddresses}:${webAppOptions.httpsPort || webAppOptions.httpPort}` +
            `, Plugins loaded: (${pluginsLoaded}/${pluginLength})`);
        }
      }, err => {
