...
const makeApiml = require('./raft').makeApiml;
const makeRaft = require('./raft').makeRaft;
  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    if (this.userConfig.node.mediationLayer.enabled
      ...
      bootstrapLogger.info(`my instance is ${instanceId}, app-server cluster size ${appServerClusterSize}`);
      const zluxInstances = yield this.apiml.waitUntilZluxClusterIsReady(appServerClusterSize);
      ...
      */
     this.apiml = yield makeApiml(this.userConfig);
     this.raft = yield makeRaft(this.apiml);
    }
  }),
