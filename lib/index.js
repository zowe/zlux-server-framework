...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {
    ...
    if (!(yield this.webServer.validateAndPreprocessConfig(wsConfig))) {
      ...
      bootstrapLogger.warn('ZWED0021W'); //bootstrapLogger.warn('Missing one or more parameters required to run.');
      bootstrapLogger.warn('ZWED0022W', (httpConfig? httpConfig.port : null), (httpsConfig? httpsConfig.port : null)); //bootstrapLogger.warn('The server requires either HTTP or HTTPS.'
        //+ ' HTTP Port given: ' + (httpConfig? httpConfig.port : null)
        //+ '. HTTPS Port given: ' + (httpsConfig? httpsConfig.port : null));
      bootstrapLogger.warn('ZWED0023W', (httpsConfig? httpsConfig.pfx : null), (httpsConfig? httpsConfig.keys : null), (httpsConfig?httpsConfig.certificates : null));  
      ...
      if ((typeof wsConfig) == 'object') {
        bootstrapLogger.warn('ZWED0024W', JSON.stringify(wsConfig, null, 2)); //bootstrapLogger.warn('config was: '+JSON.stringify(wsConfig, null, 2));
      } else {
        bootstrapLogger.warn('ZWED0025W', wsConfig); //bootstrapLogger.warn('config was: '+wsConfig);
      }
      bootstrapLogger.warn('ZWED0026W'); //bootstrapLogger.warn('All but host server and config file parameters'
    ...
    */
    ...
    this.pluginLoader.on('pluginAdded', util.asyncEventListener(event => {
      ...
      return this.pluginLoaded(event.data).then(() => {
        if (!process.clusterManager || process.clusterManager.getIndexInCluster() == 0) {
          pluginsLoaded++;
          installLogger.info('ZWED0030I', event.data.identifier, pluginsLoaded, event.count, pluginCount, event.count);
          ...
          if (pluginCount == event.count) {
            installLogger.info(`ZWED0031I`, `${runningInstances[0]}`, `${pluginsLoaded}`, `${event.count}`);
        ...
        installLogger.warn(`ZWED0027W`, `${event.data.identifier}`, `${err.message}`, pluginsLoaded, event.count, pluginCount, event.count);
        //installLogger.warn(`Exception occurred, plugin (${event.data.identifier}) installation skipped. `
        //                   +`Message: ${err.message}`
        //                   + " Successful: (" + pluginsLoaded + "/" + event.count + ")"
        //                   + " Attempted: (" + pluginCount + "/" + event.count + ")");
        installLogger.debug("ZWED0161I", err.stack); //installLogger.debug(err.stack);
        if (pluginCount == event.count) {
          installLogger.info(`ZWED0032I`, `${runningInstances[0]}`, `${pluginsLoaded}`, `${event.count}`);
          /*installLogger.info(`Server is ready at ` +
            `${runningInstances[0]}` +
            `, Plugins successfully loaded: (${pluginsLoaded}/${event.count})`);*/
        }
    ...
    if (this.userConfig.node.mediationLayer.enabled) {
      ...
      installLogger.info('ZWED0033I', webAppOptions.httpPort); //installLogger.info('The http port given to the APIML is: ', webAppOptions.httpPort);
      installLogger.info('ZWED0034I', webAppOptions.httpsPort); //installLogger.info('The https port given to the APIML is: ', webAppOptions.httpsPort);
      installLogger.info('ZWED0035I', apimlConfig); //installLogger.info('The zlux-apiml config are: ', apimlConfig);
