...
const SyncClient = require('./sync-client').SyncClient;
  ...
  start: Promise.coroutine(function*() {
    ...
    */
    ...
    if (this.userConfig.node.mediationLayer.enabled
      ...
      const port = webAppOptions.httpsPort;
      console.log(`my port is ${port}, peers: ${JSON.stringify(peers)}`);
      const me = peers.findIndex(peer => peer.port === port);
      if (me !== -1) {
        raft.start(peers, me);
      }
      ...
      // this.syncClient.onConnectionFailure((masterInstance) => {
      //   console.log(`syncClient detected that Master ${masterInstance.instanceId} failed`);
      //   console.log('Switch into master');
      //   sync.setAsMaster();
      //   const masterInstanceId = masterInstance.instanceId;
      //   Promise.all([
      //     this.apiml.takeIntoService(),
      //     this.apiml.takeInstanceOutOfService(masterInstanceId)
      //   ]);
      // });
      // const zluxInstances = yield this.apiml.getZluxInstances();
      // console.log(`got zlux instances zluxInstances ${JSON.stringify(zluxInstances, null, 2)}`);
      // const upInstances = zluxInstances.filter(instance => instance.status === 'UP' && instance.overridenStatus !== 'OUT_OF_SERVICE');
      // if (upInstances.length > 0) {
      //   console.log('I am backup');
      //   const masterInstance = upInstances[0];
      //   this.syncClient.start(masterInstance);
      // } else {
