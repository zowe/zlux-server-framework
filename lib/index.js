...
Server.prototype = {
  ...
  configureApimlStorage(apimlConfig) {
    const { gatewayHost, gatewayPort } = apimlConfig.server;
    const { cachingService } = apimlConfig;
    if (typeof gatewayHost !== 'string' || typeof gatewayPort !== 'number') {
      return;
    }
    let overrideTlsOptions = false;
    let tlsOptions = { rejectUnauthorized: false };
    // cachingService.https is a temporary solution to provide a valid key/cert pair for Caching Service
    if (cachingService && cachingService.https) {
      WebServer.readTlsOptionsFromConfig(cachingService, tlsOptions);
      if (typeof tlsOptions.cert !== 'object' || typeof tlsOptions.key !== 'object') {
        bootstrapLogger.warn(`ZWED0176W`) // Failed to load client cert/key pair for Caching Service
        return;
      }
      overrideTlsOptions = true;
    }
    if (!overrideTlsOptions) {
      tlsOptions = this.webServer.getTlsOptions();
    ...
    const settings = {
      host: gatewayHost,
      port: gatewayPort,
      tlsOptions: overrideTlsOptions ? tlsOptions : tlsOptions
    };
