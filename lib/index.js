...
Server.prototype = {
  ...
  start: Promise.coroutine(function*() {
    ...
    const sessionCookieName = (mediationLayer && mediationLayer.enabled) ? `connect.sid.${zoweInstance}` : `connect.sid.${cookiePort}`;
    const webAppOptions = {
      httpPort: httpPort,
      httpsPort: httpsPort,
    ...
    */
    ...
    if (!process.clusterManager) {
      const serverConfig = webAppOptions.serverConfig;
      const mediationLayerEnabled = serverConfig.node.mediationLayer && serverConfig.node.mediationLayer.enabled;
      if (mediationLayerEnabled) {
        const apiml = makeApiml(serverConfig);
        yield apiml.setBestIpFromConfig(serverConfig.node);
        const overrides = raft.isEnabled ? {overriddenStatus: 'OUT_OF_SERVICE'} : undefined;
        yield apiml.registerMainServerInstance(overrides);
        if (raft.isEnabled) {
          raft.setStorage(this.webApp.storage);
