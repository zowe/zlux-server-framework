...
function getAuthHandler(req, authManager) {
    ...
    if (appData.service && appData.service.def) {
        ...
        const authenticationData = service.configuration.getContents(['authentication.json']);
...
function getRelevantHandlers(authManager, body) {
    ...
    if (body && body.categories) {
        ...
        handlers = handlers.filter((h) => authCategories.hasOwnProperty(h.pluginDef.authenticationCategory));
    }
    return handlers;
}
class AuthResponse {
...
module.exports = function (authManager) {
    const _authenticateOrRefresh = BBPromise.coroutine(function* (req, res, type) {
        let functionName;
        ...
        try {
            ...
            const authServiceHandleMaps = req[`${UNP.APP_NAME}Data`].webApp.authServiceHandleMaps;
            for (const handler of handlers) {
                ...
                const handlerResult = yield handler[functionName](req, authPluginSession);
    ...
    return {
        ...
        middleware: BBPromise.coroutine(function* (req, res, next) {
            try {
