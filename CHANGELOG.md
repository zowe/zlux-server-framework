...
- Security: Modifying `instance`- and `site`-scope plugin configuration through the configuration dataservice now requires RBAC to be enabled. Previously, with RBAC disabled (the shipped default), any authenticated user could write or delete server-wide (`instance`) or site-wide (`site`) configuration for any plugin. PUT/DELETE requests to these scopes are now rejected with HTTP 403 (ZWED0145E) unless `components.app-server.dataserviceAuthentication.rbac` is `true`. Reads and `user`-scope writes are unaffected.
